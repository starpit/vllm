...
(bartowski), Qwen3-0.6B (unsloth) — all coherent.
...
* **Audit other archs end-to-end.** With P2 landed, each arch's
  GGUF support is a small `quantizations.json` edit. Tasks #3-#7
  carry the concrete JSON spec to write per arch (Phi3 fused
  qkv/gate-up; Gemma2/3 pre/post norms + sliding window; Granite
  multipliers; CommandR; DeepSeek V2/V3 MLA + MoE — V2/V3's
  bespoke MLA dim derivation isn't expressible as pure data and
  needs follow-up).
