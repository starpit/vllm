...
| P16 — A/B vs MLX + perf gate | partial baseline | `efb2983b7`, `c0bec10b7`, `717664b58`, `8bf6388f8`, `2444c844c` | Metal-runtime perf overhaul lands: residency-set fix unblocks batched ICB, batched ICB becomes default, then one encoder per forward replaces the per-step end+reopen pattern. Llama-3.2-1B-4bit decode 27 tok/s → 110 tok/s; Llama-3.2-3B-4bit ~43 tok/s. **Open gaps observed against `vllm ferrite info` cuda L40s baseline**: (a) metal lacks any `Fused Qkv*RopeCache*Impl` (decode + prefill) — both BF16 dense and affine paths show 3 separate Q/K/V matmuls + `RopeAppend` instead of one fused dispatch (cuda has dense `FusedQkvRopeCacheImpl` + Cutlass/Marlin/Bnb4/Ggml/Fp8 quant variants); (b) affine q-MLP runs 3 dispatches vs cuda's 1 `FusedGateUpSiluMul` (gated on P12 branch (ii)); (c) no metal `FusedGemmAdd` for the output-proj + residual chain. Concrete loop-body op counts on Llama-3.2-3B-4bit: cuda L40s TP=8 prefill loop = 9 ops; metal affine prefill loop = 13 ops. **Startup-time gap on `mlx-community/Llama-3.2-3B-Instruct-4bit`** (revised after `8bf6388f8` load-routing diagnostics + `2444c844c` candidate-(1) relaxation): cold `try_load` ≈ **1.34s** pre-fix → measured 411–990 ms across 5 warm runs post-fix; routing post-fix is `zero-copy 451 calls / 191.8 MiB (all dtype-relaxed) | memcpy 198 calls / 1563.9 MiB`. The F16 scales/biases/RMSNorm-gain population (451 calls, ~192 MiB) now bypasses memcpy via `alloc_and_copy_host_aligned(min_align = dtype.size_bytes())`. **Structural floor remaining**: 198 U32 packed-weight memcpy calls @ 1.56 GiB — those bindings need 4-byte alignment but the tensors land at `mod 4 == 2`. Closing that requires candidate (2)/(3) from `project_metal_safetensors_alignment`: register-time pre-aligned bulk copy (shift by 14 bytes so tensor pointers land at `mod 16 == 0`) or MTLBlit-DMA copy at ~30 GB/s. Formal A/B vs `mlx_lm.generate` + per-shape kernel benchmarks still owe. Tracking: `project_metal_fused_qkv_handoff` for QKV-fusion, `project_metal_safetensors_alignment` for the U32 memcpy floor. |
