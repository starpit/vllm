...
| P10b — in-register `<T_act, T_scale>` cast (regression repair) | pending | — | **Repays a documented deferral.** `INT4_PARITY_PROBES.md:259` recorded `Decision: in-register cast` with a microbench (`microbench_int4_cast/`) showing the cost delta is ±0.25% (in the noise) and listed concrete reasons (zero ITL cost, smaller storage, simpler loader, matches MLX). The decision was never implemented — P1-P6 shipped with the kernels templated on a single `T` matching MLX, and the loader's `set_target_dtype(BF16)` (`vllm-executor/src/ferrite_worker.rs:5300`) casts the safetensors F16 scales/biases (and RMSNorm weights, and embedding scales/biases) into BF16 at load time. F16→BF16 is **lossy** (10-bit → 7-bit mantissa) so this is a precision regression, not just a perf one — plausible contributor to the late-token MLX-vs-ferrite drift the P10 baseline measured. Scope: extend `template <typename T, …>` → `template <typename T_act, typename T_scale, …>` across `quantized_qmv.metal` / `quantized_qmm.metal` / `quantized_qvm.metal` / `quantized_dequantize.metal` + the corresponding host-side `MetalAffine*` dispatchers + pipeline cache key + lowering; remove `set_target_dtype` coverage for quant scales/biases + RMSNorm gains + embedding scales/biases (only dense activation-path tensors should still cast). Validation: re-run `test_metal_correctness_llama_3_2_1b_mlx_4bit` and tighten `prefix_token_match` toward the P10 ≥ 64-token bar. |
