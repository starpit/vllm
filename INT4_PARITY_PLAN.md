...
| P10c — RMSNorm + dense F16→BF16 cast cleanup | pending | — | Out-of-scope follow-up to P10b. After P10b the only remaining `Cast weight: F16 → BF16` log lines are the 16 × 2 RMSNorm gains + final norm + post-attention norms (33 × 2048 elements for Llama-3.2-1B). RMSNorm hasn't been profiled as a precision contributor like the quant scales were, but the same `take_keep_dtype` pattern could land here. Likely a smaller perf-vs-precision tradeoff than P10b's qmv hot path; pick up after the activation-dtype alignment work narrows P10 prompt 7 drift. |
