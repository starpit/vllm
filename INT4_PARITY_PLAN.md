...
| P3-P4 C3 — splitk reduce kernel | ✅ kernel | tbd | `splitk_reduce_sum_<dtype>` shader + `MetalSplitKReduce` dispatcher + bf16/f16 parity tests; lowering integration + scratch slot allocation deferred to C4 |
| P3-P4 C4 — solver Impl + macro flip + FUF revert + SiluMul + Llama-1B verify | pending | — | `MetalAffineQmmImpl` matching Gemm-on-Affine + new `Instruction::SiluMul` (FusedGateUpSiluMul rejects Affine, decomposed q-MLP per plan P12 branch (i)) + extend `MetalFusedGateUpSiluMulImpl::matches`/`fan_out` to emit decomposed for Affine + macro flips `LinearAffine` → `load_affine_quant` + revert `fuf.rs` Affine→Dense downgrade + scratch-slot allocation for SplitK + lowering switch to SplitK when `pick_qmm_t_kernel` says so |
