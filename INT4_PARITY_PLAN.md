...
| P16 — A/B vs MLX + perf gate | partial baseline | `efb2983b7`, `c0bec10b7`, `717664b58`, `8bf6388f8` | Metal-runtime perf overhaul lands: residency-set fix unblocks batched ICB, batched ICB becomes default, then one encoder per forward replaces the per-step end+reopen pattern. Llama-3.2-1B-4bit decode 27 tok/s → 110 tok/s; Llama-3.2-3B-4bit ~43 tok/s. **Open gaps observed against `vllm ferrite info` cuda L40s baseline**: (a) metal lacks any `Fused Qkv*RopeCache*Impl` (decode + prefill) — both BF16 dense and affine paths show 3 separate Q/K/V matmuls + `RopeAppend` instead of one fused dispatch (cuda has dense `FusedQkvRopeCacheImpl` + Cutlass/Marlin/Bnb4/Ggml/Fp8 quant variants); (b) affine q-MLP runs 3 dispatches vs cuda's 1 `FusedGateUpSiluMul` (gated on P12 branch (ii)); (c) no metal `FusedGemmAdd` for the output-proj + residual chain. Concrete loop-body op counts on Llama-3.2-3B-4bit: cuda L40s TP=8 prefill loop = 9 ops; metal affine prefill loop = 13 ops. **Startup-time gap on `mlx-community/Llama-3.2-3B-Instruct-4bit`** (revised after `8bf6388f8` load-routing diagnostics — supersedes earlier 280ms estimate): cold `try_load` ≈ **1.34s**, warm ≈ **460ms**. Root cause is **safetensors mmap alignment**: 648 of 648 tensor pointers land at file-offset `mod 16 == 2` (exactly 1 trailing zero — alignment histogram `[0, 648, 0, …]`), permanently failing `MetalAllocator::MIN_BIND_ALIGN = 16`, so **0/649 calls hit the mmap zero-copy fast path and all 1.76 GiB falls through to arena memcpy**. The unpacked-MLP recipe in `project_metal_unpacked_mlp_next` would not help — `MetalFusedGateUpSiluMulImpl::required_weights` returns per-prefix accessors on the affine path (no `load_dense_concat_packed`), so the imagined 200ms pack memcpy on 3B-4bit doesn't actually exist. Real candidates now: per-dtype binding-alignment relaxation (F16 → 2-byte saves ~250 MB), pre-aligned single-buffer copy on `register_mmap` (one shot vs N small memcpys), MTLBlit-DMA copy at ~30 GB/s, or write-time safetensors realignment (file-format change). Formal A/B vs `mlx_lm.generate` + per-shape kernel benchmarks still owe. Tracking: `project_metal_fused_qkv_handoff` for QKV-fusion, `project_metal_safetensors_alignment` for the new startup-time root cause. |
