...
| Vision tower, weights, registration | `ferrite-model-qwen2-vl/src/vision.rs` (Qwen2-VL) and `ferrite-model-qwen2-5-vl/src/vision.rs` (Qwen2.5-VL) — `VisionWeights`, `vision_forward`, `MultimodalForward` impl, `inventory::submit!` rows at tp ∈ {1,2,4,8}. Both crates pair with `ferrite-model-qwen2` (shared text decoder); `configs/qwen2-vl-2b.json` and `configs/qwen2.5-vl-3b.json` over there register the text forward for each arch string. |
| Shared vision host glue | `ferrite-vision/src/lib.rs` — `VisionConfig` (common geometric fields shape-derived per arch) with `build_rope_cos_sin_bf16` / `patches_from_normalized_chw` as methods, plus free-fn `build_cu_seqlens_i32`, `pad_linear_k_to_mult8`, `TraceDump`. Each VL crate carries its own arch-specific extras (Qwen2.5-VL: `VisionExtras { intermediate_size, window_size }`). |
