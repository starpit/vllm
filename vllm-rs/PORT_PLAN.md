...
> **Last updated**: 2026-02-28 (Mixtral MoE support)
...
| **8g+. Prompt logprobs** | **DONE** | `prompt_logprobs` parameter: during prefill, extract full logit tensor, compute `compute_logprobs()` for each prompt position (position 0 = None, positions 1..n = logprobs of actual prompt token). CandleWorker + MlxWorker. Propagated via `ModelRunnerOutput.prompt_logprobs_dict` → `EngineCoreOutput.new_prompt_logprobs` → `RequestState.prompt_logprobs` → API response (`ChatCompletionResponseChoice.prompt_logprobs`, `CompletionResponseChoice.prompt_logprobs`). `prompt_logprobs` field on `ChatCompletionRequest` + `CompletionRequest`. 4 unit + 1 E2E test. |
...
| **6i. Mixtral MoE** | **DONE** | `MixtralForCausalLM` (candle + MLX float + quantized). LLaMA-like GQA attention + MoE FFN on all layers (no dense/MoE alternation, no shared experts). Expert weights use w1/w2/w3 naming (w1=gate_proj, w3=up_proj, w2=down_proj); MoE module named `block_sparse_moe`. `MixtralExpertMLP` loads w1/w2/w3 directly (avoids modifying shared `LlamaMLP`). MLX quantized supports fused `switch_mlp` format + per-expert w1/w2/w3 format. Reuses `MlxGate` from qwen3_moe (made `pub(crate)`). 7 candle unit + 3 MLX unit tests (incl. synthetic weights E2E). |
| **6j. Phi-4 (LongRoPE + partial rotation)** | **DONE** | `Phi3ForCausalLM` now supports `rope_scaling.type="longrope"` (per-dimension frequency rescale factors, short/long caches, mscale magnitude correction) and `partial_rotary_factor` (rotary_dim < head_dim, passthrough dims). `LongRopeScaling` struct + config parsing in both candle and MLX `LlamaConfig`. `Phi3Rope` enum in MLX phi3.rs: Standard (delegates to nn::Rope) or Custom (LongRoPE). Enables `mlx-community/Unsloth-Phi-4-mini-instruct-4bit`. 7 unit + 4 E2E tests. |
...
| **13a. Embedding / Pooling** | **DONE** | `/v1/embeddings` endpoint (OpenAI-compatible). `hidden_states()` on Model trait + all candle models (9 archs) + all MLX models (17 variants). `embedding.rs` utilities (pool last-token, L2 normalize, Matryoshka truncate). `embed()` through Worker→Executor→EngineCore→Client chain. Step-loop embed channel (mpsc + oneshot). Both CandleWorker and MlxWorker. 7 unit + 7 E2E tests. (791 + 65 E2E tests) |
| **14a. Cross-request batched forward** | **DONE** | `AttentionMetadata` (per-request slicing for flat batched tensors), `BatchedKvCacheStorage` (multi-request paged KV with per-request `LayerKvHandle`), `Model::forward_batch()` trait method with default per-request fallback. `LlamaForCausalLM::forward_batch()`: batches embedding, Q/K/V projections, RoPE, norms, MLP across all requests; attention per-request via existing `attention_with_cache()`. Covers LLaMA/Mistral/Qwen2/Qwen3/Phi-3 via registry. `CandleWorker` paged path: flat tensor concat → single `forward_batch()` → split logits → per-request sampling. `MlxWorker`: deferred `eval()` across all per-request forwards for graph fusion into single Metal command buffer. 7 unit tests. |
...
| 6i | ~1,580 | 2 new + 4 mod | 10 | 724 + 57 E2E | Mixtral MoE (candle+MLX float+quantized) |
| 8g+ | ~390 | 9 mod | 4 + 1 E2E | 728 + 58 E2E | Prompt logprobs: prefill logprob extraction in CandleWorker + MlxWorker, engine propagation, API wiring |
| 13a | ~810 | 2 new + 30 mod | 12 + 7 E2E | 740 + 65 E2E | Embedding: /v1/embeddings, hidden_states(), pool+normalize, embed channel |
| **Total** | **~39,230** | **114 files** | **740 + 65 E2E** | **805** | **0 clippy errors** |
...
- **Mixtral MoE**: Token-by-token expert routing (same serialized approach as DeepSeek V2 / Qwen3 MoE). Mixtral does not renormalize top-k probabilities (unlike Qwen3 MoE's `norm_topk_prob`). No E2E tests with real models — smallest available Mixtral model is ~18 GB; synthetic weight test covers the full load→forward path.
