...
pub fn emit_model(
    ...
    quote! {
        ...
        /// Layer-0 megakernel dispatch hook (CUDA). Resolves the
        /// orchestrator's 14 source pointers from `wm`'s
        /// `WeightAccessors` impl + `ctx`'s runtime resources, then
        /// calls `ferrite_wavefront::dispatch::dispatch_one_layer_decode`
        /// on the worker's compute_stream + caching allocator.
        ///
        /// Shape-gated for Llama-3.2-1B (hidden=2048, intermediate=8192,
        /// head_dim=64, num_q_heads=32, num_kv_heads=8); other arches
        /// return `None`. Op-idx values are constants for Llama-3.2-1B's
        /// decode bucket — verified against `vllm ferrite info llama-3.2-1b`:
        /// op_idx 2 = RmsNorm (input_layernorm L=0), 4 = FusedQkvRopeCache,
        /// 6 = CutlassGemv (o_proj), 7 = FusedAddRmsNorm
        /// (post_attention_layernorm L=0), 8 = FusedGateUpSiluMul,
        /// 9 = Cublas (down_proj).
        ///
        /// `x` (post-embed hidden state) is allocated as a zero
        /// placeholder for now; wiring the real per-op embed output
        /// requires hooking the worker's arena slot 0, which is the
        /// next slice. With zero `x` the kernel runs end-to-end on
        /// real H100 hardware against real layer-0 weights — per-op
        /// forward (which the worker reads back) is unaffected.
        ///
        /// # Safety
        /// `ctx`'s tensors and `device` must outlive the dispatch.
        #[cfg(feature = "cuda")]
        #[allow(clippy::too_many_arguments)]
        #[allow(unused_variables)]
        pub unsafe fn wavefront_megakernel_layer0_dispatch(
            wm: &Weights,
            ctx: &::ferrite_forward::ForwardCtx,
            device: &mut ::ferrite_cuda_core::device::GpuDevice,
            num_tokens: u64,
        ) -> ::core::option::Option<()> {
            use ::ferrite_forward::{CanonicalParams, WeightAccessors};

            // Decode-only: m=1.
            if num_tokens != 1 {
                return ::core::option::Option::None;
            }
            // Llama-3.2-1B shape gate.
            if <Weights as CanonicalParams>::HIDDEN_SIZE != 2048
                || <Weights as CanonicalParams>::INTERMEDIATE_SIZE != 8192
                || <Weights as CanonicalParams>::HEAD_DIM != 64
                || <Weights as CanonicalParams>::NUM_Q_HEADS != 32
                || <Weights as CanonicalParams>::NUM_KV_HEADS != 8
            {
                return ::core::option::Option::None;
            }
            ...
            // Layer-0 weight resolution. `LinearLayer` is an enum;
            // `.dense_weight()` extracts the underlying `GpuTensor` for
            // the dense BF16 case (Llama-3.2-1B). Quantized variants
            // are filtered out by the shape gate above (only the dense
            // base canonical lands here for layer pointer resolution
            // — gptq/awq/fp8 siblings hit the gate via different
            // INTERMEDIATE_SIZE / HEAD_DIM constants when their decode
            // re-shapes, OR are filtered by future per-canonical
            // additions).
            let rms_w0 = wm.rms_norm_at(bucket, 2, 0, 0).weight.raw_ptr();
            let qkv_packed = wm.linear_at(bucket, 4, 0, 0).dense_weight().raw_ptr();
            // Packed qkv: rows 0..2048 = q, rows 2048..2560 = k,
            //             rows 2560..3072 = v. Each row = 2048 bf16 = 4096 B.
            let q_w = qkv_packed;
            let k_w = unsafe { qkv_packed.add(2048usize * 2048usize * 2) };
            let v_w = unsafe { qkv_packed.add((2048usize + 512usize) * 2048usize * 2) };
            // CosSin returns a single packed GpuTensor; cos/sin are
            // both views into it. The orchestrator's RoPE compute
            // body reads cos[0..head_dim] and sin[0..head_dim] from
            // their own buffers — splitting the packed table is a
            // follow-up. For now both pointers reference the same
            // base, which produces wrong RoPE rotation but doesn't
            // crash the kernel.
            let cos_sin = wm.cos_sin_at(bucket, 4, 0, 0);
            let cos_ptr = cos_sin.raw_ptr();
            let sin_ptr = cos_sin.raw_ptr();
            let k_cache_ptr = ctx.kv_cache.k_cache(0).raw_ptr() as *mut u8;
            let v_cache_ptr = ctx.kv_cache.v_cache(0).raw_ptr() as *mut u8;
            let o_w = wm.linear_at(bucket, 6, 0, 0).dense_weight().raw_ptr();
            let rms_w1 = wm.rms_norm_at(bucket, 7, 0, 0).weight.raw_ptr();
            let gate_up_packed = wm.linear_at(bucket, 8, 0, 0).dense_weight().raw_ptr();
            // gate (rows 0..8192) and up (rows 8192..16384) packed.
            let gate_w = gate_up_packed;
            let up_w = unsafe { gate_up_packed.add(8192usize * 2048usize * 2) };
            let down_w = wm.linear_at(bucket, 9, 0, 0).dense_weight().raw_ptr();

            // x (post-embed hidden state) — zero placeholder for now.
            let stream = device.compute_stream;
            let x_bytes_aligned = (2048usize * 2 + 127) & !127usize;
            let x_ptr = device.caching.alloc(x_bytes_aligned);
            if x_ptr.is_null() {
                return ::core::option::Option::None;
            }
            let _ = unsafe {
                ::ferrite_cuda_core::driver::memset_d8(
                    x_ptr, 0, x_bytes_aligned, stream,
                )
            };

            let source_ptrs: [*mut ::core::ffi::c_void; 14] = [
                x_ptr as *mut ::core::ffi::c_void,
                rms_w0 as *mut ::core::ffi::c_void,
                q_w as *mut ::core::ffi::c_void,
                k_w as *mut ::core::ffi::c_void,
                v_w as *mut ::core::ffi::c_void,
                cos_ptr as *mut ::core::ffi::c_void,
                sin_ptr as *mut ::core::ffi::c_void,
                k_cache_ptr as *mut ::core::ffi::c_void,
                v_cache_ptr as *mut ::core::ffi::c_void,
                o_w as *mut ::core::ffi::c_void,
                rms_w1 as *mut ::core::ffi::c_void,
                gate_w as *mut ::core::ffi::c_void,
                up_w as *mut ::core::ffi::c_void,
                down_w as *mut ::core::ffi::c_void,
            ];

            let caching_ptr: *mut ::ferrite_cuda_core::alloc::CachingAllocator =
                &mut device.caching as *mut _;
            let alloc_fn = move |bytes: usize| -> *mut u8 {
                // SAFETY: `device` exclusively borrowed for fn duration.
                let caching = unsafe { &mut *caching_ptr };
                let p = caching.alloc(bytes);
                if !p.is_null() {
                    let _ = unsafe {
                        ::ferrite_cuda_core::driver::memset_d8(p, 0, bytes, stream)
                    };
                }
                p
            };
            ...
            let _arena = unsafe {
                ::ferrite_forward::wavefront_cuda::dispatch::dispatch_one_layer_decode(
                    &source_ptrs,
                    1u32,
...
fn emit_shim_model(
    ...
    quote! {
        ...
        pub use super::#canonical::{
            forward, forward_backbone, wavefront_megakernel_layer0_dispatch,
        };
