...
// Mega dispatch helpers live in `ferrite-megakernel::codegen` —
// the proc-macro just orchestrates per-canonical walks and feeds
// `Instruction`s + `&mut MegaDispatchState` to the dispatch fn.
// Pulling these out of this proc-macro into the mega crate is
...
fn emit_mega_artifacts_inline(
    ...
    for (wp, (lowered, _num_slots, _backbone_slot, _terminal_slot)) in canonical_lowered {
        ...
        match emit_canonical_build_fn(
            ...
            Err(reason) => {
                skipped_count += 1;
                eprintln!("ferrite-megakernel: skipped {canonical_name}: {reason} (host fallback)");
            }
        }
    }
...
// MegaDispatchState (formerly local `PhaseCState`) lives in
// `ferrite_megakernel::codegen` along with the rest of the
// dispatch core. Imported at the top of this file.

/// Emit `fn build_mega_tape_<canonical>() -> ::ferrite_megakernel::ir::MegaTape`
...
fn emit_canonical_build_fn(
    ...
    Ok(quote! {
        ...
        /// `const { assert!(...) }` blocks in `ferrite_megakernel::ir`'s
        /// const-generic primitives. Bad const args → rustc E0080.
        #[allow(dead_code, clippy::let_and_return)]
        pub fn #fn_name() -> ::ferrite_megakernel::ir::MegaTape {
            let mut b: ::ferrite_megakernel::ir::MegaTapeBuilder<
                #num_pages_lit, #num_warps_lit, #page_size_lit, #scratch_lit, #num_edges_lit,
            > = ::ferrite_megakernel::ir::MegaTapeBuilder::new();
            #body
...
// moved to `ferrite_megakernel::codegen`.
