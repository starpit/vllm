...
mod tests {
    ...
    fn qwen2_vl_vision_weights_manifest_anchors_on_vision_bounds() {
        ...
        let qkv = manifest
            .lookup(&["attn", "qkv"])
            .expect("attn.qkv in manifest");
        // `[3 * vision_embed_dim, vision_embed_dim]` — packed Q/K/V
        // out × in.
        assert_eq!(qkv.len(), 2);

        let merger_proj = manifest
            .lookup(&["merger", "mlp", "2"])
            .expect("merger.mlp.2 in manifest");
        // `[d_model, vision_merge_hidden]` — encoder output → text
        // hidden.
        ...
        // `[vision_embed_dim, vision_in_features]`.
