...
pub fn emit_cu_source(canonical_name: &str, dims: &KvmDims) -> String {
    let vendor_dir = "/home/moosevan/Megakernels/demos/cross-gpu-llama";
    format!(
        "// SPDX-License-Identifier: Apache-2.0
// Auto-generated by ferrite interpreter::kvm for canonical `{canonical_name}`.
// DO NOT EDIT BY HAND.

#define LLAMA_NUM_LAYERS {num_layers}
#define LLAMA_HIDDEN_DIM {hidden_dim}
#define LLAMA_INTERMEDIATE_DIM {intermediate_dim}
#define LLAMA_HEAD_DIM {head_dim}
#define LLAMA_NUM_ATTENTION_HEADS {num_attention_heads}
#define LLAMA_NUM_KV_HEADS {num_kv_heads}
#define LLAMA_KV_PAGE_SIZE {kv_page_size}
#define LLAMA_PREFILL_KV_BLOCK_SIZE 128
#define LLAMA_DECODE_KV_BLOCK_SIZE 16
#define LLAMA_MATMUL_OUT_BLOCK_SIZE {matmul_out_block_size}
#define LLAMA_MATMUL_BATCH_BLOCK_SIZE {matmul_batch_block_size}
#define LLAMA_NUM_DEVICES {num_devices}
#define SM_COUNT {sm_count}

#include \"{vendor_dir}/llama.cuh\"
#include \"{vendor_dir}/batched_rms_norm.cu\"
#include \"{vendor_dir}/qkv_rope_append.cu\"
#include \"{vendor_dir}/attention_decode.cu\"
#include \"{vendor_dir}/attention_prefill.cu\"
#include \"{vendor_dir}/matmul_adds.cu\"
#include \"{vendor_dir}/gate_silu.cu\"
#include \"{vendor_dir}/up_matmul.cu\"
#include \"{vendor_dir}/lm_head.cu\"
#include \"{vendor_dir}/inc_barriers.cu\"
#include \"{vendor_dir}/all_device_barrier.cu\"
...
using tk_megakernel_t = megakernel::mk<llama_config, llama_70b_globals,
    ...
    ops::all_device_barrier_op>;

extern \"C\" int tk_megakernel_{canonical_name}_launch(
    void* d_bar, int bar_d0, int bar_d1, int bar_d2, int bar_d3,
    void* d_instructions, int num_instructions,
    void* d_timings, int num_timing_rows,
    void* d_global_instruction_index,
    void* d_qkv_weights, int qkv_R,
    void* d_attn_norm_weights,
    void* d_o_weights, int o_R,
    void* d_mlp_norm_weights,
    void* d_up_weights, int up_R,
    void* d_gate_weights, int gate_R,
    void* d_down_weights, int down_R,
    void* d_lm_head_norm_weights,
    void* d_lm_head_weights, int lm_head_R,
    void* d_k_cache, int kv_total_pages, int kv_page_size_runtime,
    void* d_v_cache,
    void* d_rope_cos, int max_pos,
    void* d_rope_sin,
    void* d_hidden_states, int batch_size_arg,
    void* d_rms_rope_intermediates,
    void* d_rms_gate_intermediates,
    void* d_q_post_rope,
    void* d_attn_out,
    void* d_silu_out,
    void* d_rms_lm_head_intermediates,
    void* d_logits, int vocab_size_arg,
    void* d_position_ids, int num_position_ids,
    void* d_kv_append_indices,
    void* d_prefill_qo_indptr, int num_prefill_qo,
    void* d_prefill_kv_indptr,
    void* d_prefill_kv_indices, int num_prefill_kv_indices,
    void* d_prefill_kv_last_page_len,
    void* d_decode_kv_indptr, int num_decode_seqs,
    void* d_decode_kv_indices, int num_decode_kv_indices,
    void* d_decode_kv_last_page_len,
    float attn_scale, float rms_norm_eps,
    int num_pages, int num_prefill_tokens, int dev_idx,
    void* raw_stream
) {{
    using kittens::bf16;
    ...
    llama_70b_globals g {{
        {{ static_cast<uint*>(d_bar), (size_t)bar_d0, (size_t)bar_d1, (size_t)bar_d2, (size_t)bar_d3 }},
        {{ static_cast<int*>(d_instructions), nullptr, nullptr, (size_t)num_instructions, nullptr }},
        {{ static_cast<int*>(d_timings), nullptr, nullptr, (size_t)num_timing_rows, nullptr }},
        {{ static_cast<int*>(d_global_instruction_index), nullptr, nullptr, nullptr, nullptr }},
        {{ static_cast<bf16*>(d_qkv_weights), nullptr, LLAMA_NUM_LAYERS, qkv_R, nullptr }},
        {{ static_cast<bf16*>(d_attn_norm_weights), nullptr, nullptr, LLAMA_NUM_LAYERS, nullptr }},
        {{ static_cast<bf16*>(d_o_weights), nullptr, LLAMA_NUM_LAYERS, o_R, nullptr }},
        {{ static_cast<bf16*>(d_mlp_norm_weights), nullptr, nullptr, LLAMA_NUM_LAYERS, nullptr }},
        {{ static_cast<bf16*>(d_up_weights), nullptr, LLAMA_NUM_LAYERS, up_R, nullptr }},
        {{ static_cast<bf16*>(d_gate_weights), nullptr, LLAMA_NUM_LAYERS, gate_R, nullptr }},
        {{ static_cast<bf16*>(d_down_weights), nullptr, LLAMA_NUM_LAYERS, down_R, nullptr }},
        {{ static_cast<bf16*>(d_lm_head_norm_weights), nullptr, nullptr, 1, nullptr }},
        {{ static_cast<bf16*>(d_lm_head_weights), nullptr, 1, lm_head_R, nullptr }},
        {{ static_cast<bf16*>(d_k_cache), (size_t)kv_total_pages, (size_t)kv_page_size_runtime, nullptr, nullptr }},
        {{ static_cast<bf16*>(d_v_cache), (size_t)kv_total_pages, (size_t)kv_page_size_runtime, nullptr, nullptr }},
        {{ static_cast<float*>(d_rope_cos), nullptr, nullptr, (size_t)max_pos, nullptr }},
        {{ static_cast<float*>(d_rope_sin), nullptr, nullptr, (size_t)max_pos, nullptr }},
        {{ static_cast<bf16*>(d_hidden_states), nullptr, nullptr, (size_t)batch_size_arg, nullptr }},
        {{ static_cast<bf16*>(d_rms_rope_intermediates), nullptr, nullptr, (size_t)batch_size_arg, nullptr }},
        {{ static_cast<bf16*>(d_rms_gate_intermediates), nullptr, nullptr, (size_t)batch_size_arg, nullptr }},
        {{ static_cast<bf16*>(d_q_post_rope), nullptr, nullptr, (size_t)batch_size_arg, (size_t)(LLAMA_NUM_ATTENTION_HEADS * LLAMA_HEAD_DIM) }},
        {{ static_cast<bf16*>(d_attn_out), nullptr, nullptr, (size_t)batch_size_arg, nullptr }},
        {{ static_cast<bf16*>(d_silu_out), nullptr, nullptr, (size_t)batch_size_arg, nullptr }},
        {{ static_cast<bf16*>(d_rms_lm_head_intermediates), nullptr, nullptr, (size_t)batch_size_arg, nullptr }},
        {{ static_cast<bf16*>(d_logits), nullptr, nullptr, (size_t)batch_size_arg, (size_t)vocab_size_arg }},
        {{ static_cast<int*>(d_position_ids), nullptr, nullptr, nullptr, (size_t)num_position_ids }},
        {{ static_cast<int*>(d_kv_append_indices), nullptr, nullptr, nullptr, (size_t)num_position_ids }},
        {{ static_cast<int*>(d_prefill_qo_indptr), nullptr, nullptr, nullptr, (size_t)num_prefill_qo }},
        {{ static_cast<int*>(d_prefill_kv_indptr), nullptr, nullptr, nullptr, (size_t)num_prefill_qo }},
        {{ static_cast<int*>(d_prefill_kv_indices), nullptr, nullptr, nullptr, (size_t)num_prefill_kv_indices }},
        {{ static_cast<int*>(d_prefill_kv_last_page_len), nullptr, nullptr, nullptr, (size_t)num_prefill_qo }},
        {{ static_cast<int*>(d_decode_kv_indptr), nullptr, nullptr, nullptr, (size_t)num_decode_seqs }},
        {{ static_cast<int*>(d_decode_kv_indices), nullptr, nullptr, nullptr, (size_t)num_decode_kv_indices }},
        {{ static_cast<int*>(d_decode_kv_last_page_len), nullptr, nullptr, nullptr, (size_t)num_decode_seqs }},
        attn_scale, rms_norm_eps,
        num_pages, batch_size_arg, num_prefill_tokens,
        dev_idx,
    }};

    auto kernel_ptr = (void*)&megakernel::mk<llama_config, llama_70b_globals,
        ops::attn_norm_op, ops::qkv_rope_append_op, ops::attention_decode_op,
        ops::attention_prefill_op, ops::o_proj_op, ops::mlp_norm_op,
        ops::gate_silu_op, ops::up_matmul_op, ops::downproj_op,
        ops::lm_head_norm_op, ops::lm_head_op, ops::barrier_inc_op,
    ...
    cudaError_t err = cudaLaunchCooperativeKernel(
        kernel_ptr, g.grid(), g.block(), args, g.dynamic_shared_memory(), stream);
    return (int)err;
}}
",
        ...
        kv_page_size = dims.kv_page_size,
        matmul_out_block_size = dims.matmul_out_block_size,
        matmul_batch_block_size = dims.matmul_batch_block_size,
        num_devices = dims.num_devices,
        sm_count = dims.sm_count,
    )
}
