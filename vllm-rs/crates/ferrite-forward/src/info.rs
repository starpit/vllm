...
impl<W> Instruction<W> {
    ...
    pub fn normalize(&self) -> NormalizedStep {
        ...
        let (kind, fields): (&'static str, Vec<NormalizedField>) = match *self {
            ...
            Instruction::BiasAdd(out_slot, layer, _wf) => (
                "BiasAdd",
                vec![
                    F::Slot(out_slot),
                    F::Layer(layer),
                    F::LayerKind("GpuTensor"),
                ],
            ),
            Instruction::Silu(slot) => ("Silu", vec![F::Slot(slot)]),
            Instruction::Gelu(slot) => ("Gelu", vec![F::Slot(slot)]),
            Instruction::Mul(a_slot, b_slot) => ("Mul", vec![F::Slot(a_slot), F::Slot(b_slot)]),
            #[cfg(feature = "nccl")]
