...
mod dispatcher {
    ...
    pub trait FerriteWeights: Send + Sync {
        ...
        /// to also run the orchestrator-emitted single-CTA persistent
        /// megakernel against the same runtime context. Mirrors metal's
        /// `pool::run_wavefront_mega` pattern: the per-op `forward_backbone`
        /// stays the trusted output, and the megakernel runs alongside
        /// for diagnostic / A-B / future-takeover purposes.
        ///
        /// Default impl shape-checks the arch (Llama-3.2-1B-shaped
        /// today: hidden=2048, head_dim=64, m=1 decode) and dispatches
        /// the orchestrator's `tk_decode_one_layer` kernel with
        /// synthetic zero-filled source buffers. Output is garbage —
        /// the per-op `forward_backbone` is the trusted result the
        /// worker reads — but the megakernel FIRES on real H100
        /// hardware through real CUDA streams + the worker's caching
        /// allocator. Wiring real layer-0 weight pointers is a
        /// follow-up: each weight's device pointer comes from
        /// `WeightAccessors::linear_at(decode_bucket, op_idx, slot,
        /// layer=0).weight.raw_ptr()`, but the (bucket, op_idx) tuple
        /// for each source needs to be threaded through from macro-
        /// emit time.
        ///
        /// # Safety
        ...
        unsafe fn wavefront_megakernel_dispatch_cuda(
            ...
            // Decode-only (m=1). Existing fixture is single-head; real
            // Llama is GQA — the kernel still LAUNCHES correctly on
            // the bigger hidden, but only the first head_dim columns
            // get touched. Acceptable for the wiring milestone.
            if num_tokens != 1 {
                return None;
            }
            // Llama-3.2-1B-shaped arches only (the orchestrator fixture
            // hardcodes hidden=2048, intermediate=8192, head_dim=64).
            ...
            let input = ferrite_wavefront::fixtures::one_layer_input();
            let raw_sizes = ferrite_wavefront::fixtures::buf_byte_sizes(&input);
            let stream = device.compute_stream;

            // Allocator closure: alloc + zero via the worker's caching
            // allocator + the compute_stream's memset. Captured by the
            // dispatch call; the closure outlives the dispatch (both
            // bounded by `&mut device` here).
            let caching_ptr: *mut ferrite_cuda_core::alloc::CachingAllocator =
                &mut device.caching as *mut _;
            let alloc_fn = |bytes: usize| -> *mut u8 {
                // SAFETY: `device` is exclusively borrowed for the
                // entire override body; the raw pointer reborrow stays
                // inside that scope.
            ...
            // Allocate 14 zero-filled source buffers via the same
            // closure. Synthetic-data shortcut — a follow-up commit
            // threads real WeightAccessor pointers through and
            // replaces these with live weight tensors. Buffers leak
            // into the caching allocator's pool (no explicit free);
            // pool reuses them on next dispatch, bounded memory.
            let mut source_bufs: Vec<*mut u8> = Vec::with_capacity(14);
            for &n in &raw_sizes[..14] {
                let bytes = (n.max(128).div_ceil(128)) * 128;
                let p = alloc_fn(bytes);
                if p.is_null() {
                    return None;
                }
                source_bufs.push(p);
            }
            let source_ptrs: [*mut std::ffi::c_void; 14] = std::array::from_fn(|i| {
                source_bufs[i] as *mut std::ffi::c_void
            });
            ...
            // Visibility log: a real forward_backbone call has reached
            // the orchestrator-emitted megakernel and dispatched it on
            // the worker's CUDA stream. Removable once an actual
            // correctness check is wired up.
            eprintln!(
                "[wavefront-cuda] dispatched tk_decode_one_layer for arch={} \
                 hidden={} num_tokens={}",
                self.arch_name(),
                self.hidden_size(),
                num_tokens,
