...
pub fn build_decode_metadata(
    ...
    for seq in 0..num_seqs {
        ...
        kv_last_page_len.push(if last_len == 0 && seq_len > 0 {
            kv_page_size as i32
        } else {
            last_len as i32
        });
    ...
    let alloc_and_upload = |data: &[i32],
     ...
     -> (*mut u8, usize) {
        if data.is_empty() {
            ...
            unsafe {
                driver::memset_d8(buf, 0, 4, stream).expect("dummy zero");
            }
            return (buf, 0);
        }
...
pub fn build_prefill_metadata(
    ...
    for seq in 0..num_seqs {
        ...
        kv_last_page_len.push(if last_len == 0 && seq_len_k > 0 {
            kv_page_size as i32
        } else {
            last_len as i32
        });
