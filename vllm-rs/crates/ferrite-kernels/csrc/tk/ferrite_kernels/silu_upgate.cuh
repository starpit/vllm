...
// Ferrite-owned TK 2.0 silu_upgate op — 4 warp-role functions.
//
// Math (the fused MLP "gate / up / silu / mul" block):
//   gate[row] = sum_k W_gate[row, k] * x[k]
//   up  [row] = sum_k W_up  [row, k] * x[k]
//   out [row] = silu(gate[row]) * up[row]
//             = (gate[row] / (1 + exp(-gate[row]))) * up[row]
//
// TK-canonical shape (ported from
// third_party/thunderkittens/tests/vm/llama_official/upgate.cu):
// each CTA produces 16 consecutive output rows per block-iter via
// two concurrent register-tile matvecs (gate + up) into separate
// sv_fl<16> accumulators, followed by an elementwise silu-gate in
// registers. Output is 16 bf16 scalars TMA-stored to gmem. ferrite
// variant drops the pre-rms (separate `fused_add_rms_norm` op
// handles it upstream in the mega tape).
//
// K-inner chunk loop — identical pattern to `gemv_bf16.cuh`: each
// warp loops its `K_PER_WARP = HIDDEN_DIM / NCW` reduction slice in
// `pick_chunk_cols(K_PER_WARP)`-wide sub-chunks (≤ 512), calling
// `ferrite::tk::matvec` once per (chunk, weight) → NUM_K_CHUNKS × 2
// inner matvec calls per block-iter per warp. The gate accumulator
// and up accumulator are separate `sv_fl<16>` scratch slots; both
// matvecs atomicAdd their per-warp partials into their respective
// slots for the whole K dim, then the silu-gate finalise reads both.
//
// Page layout (STAGES=1 cross-block — 2 * NCW weight pages already
// pushes total pages close to 9 at NCW=4; STAGES=2 would double that
// to 17 pages × 16 KB = 272 KB past Hopper's 228 KB dynamic-shmem
// cap):
//   pages[base_stage + 0]
//       — sv_bf<HIDDEN_DIM> activation, loaded once per CTA, shared
//         across all block-iters AND all K-chunks.
//   pages[base_stage + 1 + w]              (w ∈ [0, NCW))
//       — st_bf<16, CHUNK_COLS, false> gate weight tile for warp w,
//         refilled per K-chunk by the loader.
//   pages[base_stage + 1 + NCW + w]
//       — st_bf<16, CHUNK_COLS, false> up weight tile for warp w,
//         refilled per K-chunk.
// Total pages: 1 + 2 * NCW.
//
// Scratch layout:
...
//                       8 floats; shares the fp32 slot space but is
//                       only written by warp 0 after publish).
//
// Semaphores: per-page page_ready / page_done mbarriers. Weight
// pages get refilled per K-chunk (loader waits on page_done from the
// previous chunk; consumer arrives on page_done when done).
//
// bar IDs for consumer-scoped syncs:
//   13 — zero-output-accumulators fence (before chunks begin).
//   14 — publish fence (after all chunks × both matvecs landed).
...
namespace ferrite {
namespace ops {
namespace silu_upgate {

constexpr int kActPageOff = 0;

// GROUP-PAGE layout using TileGroupLayout<NCW, K_PER_WARP=HIDDEN/NCW, PAGE_SIZE>.
// kChunkCols = ferrite_pick_chunk_cols(K_PER_WARP, PAGE_SIZE/32).
// With PAGE_SIZE=16384, NCW=8, K_PER_WARP=256: kChunkCols=256, GROUP_SIZE=2, 9 pages.
// With PAGE_SIZE=16384, NCW=4, K_PER_WARP=512: kChunkCols=512, GROUP_SIZE=1, 9 pages.
        ...
        for (int k_chunk = 0; k_chunk < NUM_K_CHUNKS_PER_WARP; ++k_chunk, ++iter) {
            ...
            if (iter >= 1) {
                ...
                for (int g = 0; g < NUM_GROUPS; ++g) {
                    kittens::wait(
                        ss.page_done[base_stage + UGL::gate_page(g * GROUP_SIZE)], prev_phase);
                    kittens::wait(
                        ss.page_done[base_stage + UGL::up_page(g * GROUP_SIZE)], prev_phase);
                }
            }
...
__device__ __forceinline__ void consumer(
    ...
    // Scratch: [2*NCW*sv_fl<16> gate+up slots][sv_bf<16> output staging]
    constexpr size_t SU_BF16_OFF = 2u * NCW * 16 * sizeof(float);
    uint8_t* scratch_base = ss.scratch;
    out_sv& my_gate_accum = *reinterpret_cast<out_sv*>(
        scratch_base + warp_in_role * 16 * sizeof(float));
    out_sv& my_up_accum = *reinterpret_cast<out_sv*>(
    ...
    for (int block = blockIdx.x; block < num_blocks; block += gridDim.x, ++block_counter) {
        // Zero this warp's exclusive gate and up slots.
        ...
        // Warp 0: reduce gate+up, compute silu(gate)*up, write staging, signal storer.
        if (warp_in_role == 0) {
            ...
            kittens::rv_fl<16> sig_rv;
            warp::mul(sig_rv, gate_rv, -1.0f);
            warp::exp(sig_rv, sig_rv);
            warp::add(sig_rv, sig_rv, 1.0f);
            warp::div(gate_rv, gate_rv, sig_rv);
            warp::mul(gate_rv, gate_rv, up_rv);
...
__device__ __forceinline__ void storer(
    ...
    for (int block = blockIdx.x; block < num_blocks; block += gridDim.x, ++block_iter) {
        // Wait for consumer warp 0 to reduce, compute silu, and fill staging area.
        kittens::wait(ss.page_done[base_stage + kActPageOff], block_iter & 1);

        // TMA store from the pre-computed bf16 staging area.
        auto& out_bf = *reinterpret_cast<kittens::sv_bf<16>*>(scratch_base + SU_BF16_OFF);
