...
// FP8 variant — Phase 7d. Struct definition is dual-mode (it only names
// `Fp8Linear` / `GpuTensor` which are dual-mode types); the impl block
// is cuda-only. Metal-side load stub lives at the bottom of the file.
...
// Metal-side stub for the FP8 variant — same rationale as the BF16
// stub above; `bail!` at runtime if anyone tries to load an FP8
// Qwen3-Next checkpoint on Metal before the kernel port lands.
#[cfg(feature = "metal")]
impl Fp8GatedAttentionLayer {
    pub fn load(
        _gw: &mut ferrite_cuda_core::weights::GpuWeights,
        ...
        anyhow::bail!(
            "Fp8GatedAttentionLayer::load not yet implemented on Metal -- \
             FP8 variant arrives after the BF16 port"
        )
    }
}
