...
// Dual-mode like `layers_moe`: struct *definitions* compile without the
// `cuda` feature (they only name dual-mode types — `GpuTensor`, `DType`,
// `Linear`), so the `Instruction::GdnAttention(W, Qwen3NextGdnLayer)`
// variant in `ferrite-forward` resolves under metal. The `impl` blocks
// that drive cudarc / `kernels::*` are individually `#[cfg(feature =
// "cuda")]`-gated below.
