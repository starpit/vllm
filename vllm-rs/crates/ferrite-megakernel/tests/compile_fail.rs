...
//! each file with the ferrite-megakernel extern, and assert the
...
//!
//! Run with `cargo test -p ferrite-megakernel --test compile_fail`.
...
/// Find the latest `libferrite_megakernel-*.rlib` in the workspace
...
fn find_lib_artifact() -> PathBuf {
    ...
    if let Ok(entries) = std::fs::read_dir(&workspace_target) {
        for e in entries.flatten() {
            ...
            if !name.starts_with("libferrite_megakernel-") || !name.ends_with(".rlib") {
                continue;
            }
    ...
    latest
        .expect("no libferrite_megakernel-*.rlib found — run `cargo build -p ferrite-megakernel` first")
        .0
}

/// Invoke `rustc` on `src_path` with the appropriate `--extern`
/// flag pointing at the latest ferrite-megakernel rlib. Returns
...
fn rustc_compile(src_path: &Path, output_bin: &Path) -> (bool, String) {
    ...
    let out = Command::new("rustc")
        ...
        .arg(output_bin)
        .arg("--extern")
        .arg(format!("ferrite_megakernel={}", lib.display()))
        .arg("-L")
