...
pub fn emit_flash_attn_fwd_paged() -> String {
    let mut s = String::with_capacity(500 * 1024);
    emit_kernel_paged(&mut s);
    s
}
...
/// Emit paged-KV flash attention kernel (d=64, non-causal).
///
/// Same structure as emit_kernel but with paged KV cache addressing.
/// Requires page_block_size >= BLOCK_N (64) so each KV block is contiguous
/// within a single physical page.
///
/// KV cache layout: K_cache[physical_page][token_in_page][head_dim] contiguous f16
///   page_stride = page_block_size * head_dim (in elements)
///   To access token t: page_idx = t / page_block_size
///                       offset = t % page_block_size
///                       physical_page = block_table[batch_idx * max_blocks_per_seq + page_idx]
///                       addr = K_cache + physical_page * page_stride * 2 + offset * head_dim * 2
///
/// Register assignments for paged params:
///   %rd14 = block_table base ptr
///   %rd15 = K_cache base ptr
///   %rd16 = V_cache base ptr
///   %r700 = page_stride (elements)
///   %r701 = page_block_size (tokens per page)
///   %r702 = max_blocks_per_seq
///   %rd17 = computed K base addr for current KV block
///   %rd18 = computed V base addr for current KV block
///   %rd19 = block_table ptr for this batch entry
fn emit_kernel_paged(s: &mut String) {
    // ─── Header ───
    s.push_str(
        r#".version 8.7
.target sm_89
...
.visible .entry flash_attn_fwd_paged(
	.param .u64 .ptr .global .align 16 param_Q,
	.param .u64 .ptr .global .align 16 param_K_cache,
	.param .u64 .ptr .global .align 16 param_V_cache,
	.param .u64 .ptr .global .align 16 param_O,
	.param .u64 .ptr .global .align 16 param_block_table,
	.param .u32 param_seq_len,
	.param .f32 param_scale,
	.param .u32 param_stride_q_batch,
	.param .u32 param_page_stride,
	.param .u32 param_page_block_size,
	.param .u32 param_max_blocks_per_seq
)
.reqntid 128
{
	.reg .pred 	%p<30>;
...
"#,
    ...
    // ─── Parameters ───
    w(s, "ld.param.b64 \t%rd1, [param_Q];");
    w(s, "ld.param.b64 \t%rd15, [param_K_cache];");
    w(s, "ld.param.b64 \t%rd16, [param_V_cache];");
    w(s, "ld.param.b64 \t%rd4, [param_O];");
    w(s, "ld.param.b64 \t%rd14, [param_block_table];");
    w(s, "ld.param.b32 \t%r1, [param_seq_len];");
    w(s, "ld.param.b32 \t%r2, [param_scale];"); // qk_scale (f32 bits)
    w(s, "ld.param.b32 \t%r3, [param_stride_q_batch];");
    w(s, "ld.param.b32 \t%r700, [param_page_stride];");
    w(s, "ld.param.b32 \t%r701, [param_page_block_size];");
    w(s, "ld.param.b32 \t%r702, [param_max_blocks_per_seq];");
    blank(s);
    ...
    // Q and O base pointers adjusted for batch/head (same as non-paged)
    w(s, "mul.lo.s32 \t%r8, %r5, %r3;");
    w(s, "mad.wide.s32 \t%rd10, %r8, 2, %rd1;"); // Q_base
    w(s, "mad.wide.s32 \t%rd13, %r8, 2, %rd4;"); // O_base
    blank(s);

    // block_table pointer for this batch entry
    // batch_idx = ctaid.y (batch*head index, but block_table is per-batch not per-head)
    // For simplicity in this initial version, we treat batch_head index as the batch index
    // (caller should set grid_y = batch, and handle heads via stride_q_batch)
    // block_table_ptr = block_table + batch_idx * max_blocks_per_seq * 4 (int32 = 4 bytes)
    w(s, "mul.lo.s32 \t%r703, %r5, %r702;"); // batch_idx * max_blocks_per_seq
    w(s, "shl.b32 \t%r704, %r703, 2;"); // * 4 bytes (int32)
    w(s, "cvt.u64.u32 \t%rd19, %r704;");
    w(s, "add.s64 \t%rd19, %rd14, %rd19;"); // block_table ptr for this batch
    blank(s);
    ...
    // ─── Load Q → smem (offset 0) with B128 swizzle ───
    // Same as non-paged: 8 rounds of 16 rows each
    for round in 0..8u32 {
        let row_base = round * 16;
        w(s, "add.s32 \t%r60, %r7, %r9;"); // block_m_start + cp_row_in_chunk
        if row_base > 0 {
    ...
    // Compute paged K base for kv_start=0 and load K[0] into KV buf 0
    emit_compute_paged_kv_base(s, "%rd15", "%rd17"); // K base for kv_start=0
    emit_cp_async_kv_paged(s, 16384, "%rd17");
    w(s, "cp.async.commit_group;");
    w(s, "cp.async.wait_group \t0;");
    w(s, "bar.sync \t0;");
    blank(s);
    ...
    // Compute current buffer offset
    w(s, "shr.u32 \t%r238, %r236, 6;"); // kv_start / 64
    w(s, "and.b32 \t%r239, %r238, 1;"); // buf index
    w(s, "shl.b32 \t%r240, %r239, 13;"); // buf * 8192
    w(s, "add.s32 \t%r241, %r240, 16384;"); // current KV smem offset
    ...
    // Pipeline: start loading next K (paged)
    w(s, "@!%p2 bra \t$L_SKIP_NEXT_K;");
    w(s, "mov.b32 \t%r250, %r236;"); // save current kv_start
    w(s, "mov.b32 \t%r236, %r237;"); // temporarily set kv_start to next
    emit_compute_paged_kv_base(s, "%rd15", "%rd17"); // K base for next kv_start
    emit_cp_async_kv_paged_dynamic(s, "%r244", "%rd17");
    w(s, "cp.async.commit_group;");
    w(s, "mov.b32 \t%r236, %r250;"); // restore kv_start
    s.push_str("$L_SKIP_NEXT_K:\n");
    blank(s);
    ...
    // ─── Load V block (paged) → current KV buffer ───
    emit_compute_paged_kv_base(s, "%rd16", "%rd18"); // V base for current kv_start
    emit_cp_async_kv_paged_dynamic(s, "%r241", "%rd18");
    w(s, "cp.async.commit_group;");
    w(s, "cp.async.wait_group \t0;");
    w(s, "bar.sync \t0;");
    blank(s);
...
/// Compute paged KV base address for current kv_start (%r236).
///
/// Given cache_base_reg (e.g. "%rd15" for K_cache or "%rd16" for V_cache),
/// computes the global address of the first element of the KV block and stores
/// it in result_reg (e.g. "%rd17" or "%rd18").
///
/// Algorithm:
///   page_idx = kv_start / page_block_size
///   offset_in_page = kv_start % page_block_size  (== kv_start - page_idx * page_block_size)
///   physical_page = block_table[page_idx]  (ld.global.b32)
///   base = cache_base + physical_page * page_stride * 2 + offset_in_page * head_dim * 2
///
/// Uses temp regs %r705..%r709 and %rd70..%rd72.
fn emit_compute_paged_kv_base(s: &mut String, cache_base_reg: &str, result_reg: &str) {
    // page_idx = kv_start / page_block_size
    w(s, "div.u32 \t%r705, %r236, %r701;"); // page_idx
    // offset_in_page = kv_start - page_idx * page_block_size
    w(s, "mul.lo.s32 \t%r706, %r705, %r701;");
    w(s, "sub.s32 \t%r707, %r236, %r706;"); // offset_in_page

    // Load physical_page = block_table[page_idx]
    // block_table address = %rd19 + page_idx * 4
    w(s, "shl.b32 \t%r708, %r705, 2;"); // page_idx * 4 bytes
    w(s, "cvt.u64.u32 \t%rd70, %r708;");
    w(s, "add.s64 \t%rd71, %rd19, %rd70;"); // &block_table[page_idx]
    w(s, "ld.global.b32 \t%r709, [%rd71];"); // physical_page

    // base = cache_base + physical_page * page_stride * 2 + offset_in_page * 128
    // page_stride is in elements, * 2 for bytes
    w(s, "mul.lo.s32 \t%r708, %r709, %r700;"); // physical_page * page_stride (elements)
    w(s, "shl.b32 \t%r708, %r708, 1;"); // * 2 bytes
    w(s, "shl.b32 \t%r706, %r707, 7;"); // offset_in_page * 128 (head_dim=64 * 2 bytes)
    w(s, "add.s32 \t%r708, %r708, %r706;"); // total byte offset
    w(s, "cvt.u64.u32 \t%rd72, %r708;");
    w(s, &format!("add.s64 \t{result_reg}, {cache_base_reg}, %rd72;"));
}

/// Emit cp.async for paged KV tile with B128 swizzle (static smem offset).
/// Uses kv_start from %r236. The base_ptr_reg is the computed paged base address.
/// Within the page, rows are contiguous (page_block_size >= 64), so we can do
/// sequential cp.async just like the non-paged version but relative to the paged base.
fn emit_cp_async_kv_paged(s: &mut String, smem_offset: u32, base_ptr_reg: &str) {
    for round in 0..4u32 {
        let row_off = round * 16;
        // Bounds check: kv_start + row_off + cp_row < seq_len
        w(s, "add.s32 \t%r60, %r236, %r9;"); // kv_start + cp_row_in_chunk
        if row_off > 0 {
            w(s, &format!("add.s32 \t%r60, %r60, {};", row_off));
        }
        ...
        // Global addr: base_ptr + row_within_block * 128 + col_bytes
        // row_within_block = row_off + cp_row_in_chunk
        w(s, "shl.b32 \t%r62, %r9, 7;"); // cp_row_in_chunk * 128
        if row_off > 0 {
            w(s, &format!("add.s32 \t%r62, %r62, {};", row_off * 128));
        }
        w(s, "add.s32 \t%r63, %r62, %r11;"); // + col_bytes
        w(s, "cvt.u64.u32 \t%rd20, %r63;");
        w(s, &format!("add.s64 \t%rd21, {base_ptr_reg}, %rd20;"));
...
/// Same as emit_cp_async_kv_paged but with dynamic smem offset in a register.
fn emit_cp_async_kv_paged_dynamic(s: &mut String, smem_off_reg: &str, base_ptr_reg: &str) {
    for round in 0..4u32 {
        let row_off = round * 16;
        w(s, "add.s32 \t%r60, %r236, %r9;");
        if row_off > 0 {
...
mod tests {
    ...
    // ═══════════════════════════════════════════════════════════════
    // Paged KV cache kernel tests
    // ═══════════════════════════════════════════════════════════════

    fn load_paged_kernel() -> String {
        emit_flash_attn_fwd_paged()
    }

    #[test]
    fn test_paged_has_entry_point() {
        let ptx = load_paged_kernel();
        assert!(ptx.contains(".entry flash_attn_fwd_paged"),
            "Must have flash_attn_fwd_paged entry point");
    }

    #[test]
    fn test_paged_has_block_table_param() {
        let ptx = load_paged_kernel();
        assert!(ptx.contains("param_block_table"),
            "Must have block_table parameter");
    }

    #[test]
    fn test_paged_instruction_counts() {
        let ptx = load_paged_kernel();
        let mma = ptx.lines().filter(|l| l.contains("mma.sync")).count();
        let ldm = ptx.lines().filter(|l| l.contains("ldmatrix")).count();
        let cpa = ptx.lines().filter(|l| l.contains("cp.async.cg")).count();
        ...
        eprintln!("Paged flash attn instruction counts: mma={}, ldmatrix={}, cp.async={}, ex2={}",
                  mma, ldm, cpa, ex2);

        // Same MMA/ldmatrix/cp.async counts as non-paged
        assert!(mma >= 16, "Need at least 16 MMA (Q@K^T + P@V), got {}", mma);
        assert!(ldm >= 8, "Need at least 8 ldmatrix, got {}", ldm);
        assert!(cpa >= 4, "Need at least 4 cp.async, got {}", cpa);
        assert!(ex2 >= 4, "Need at least 4 ex2 for softmax, got {}", ex2);
    ...
    #[test]
    fn test_paged_has_block_table_lookup() {
        let ptx = load_paged_kernel();
        // Must have ld.global for block_table lookups
        assert!(ptx.contains("ld.global.b32"),
            "Must have ld.global.b32 for block_table lookups");
    }
}
