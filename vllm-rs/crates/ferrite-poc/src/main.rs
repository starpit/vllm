...
fn main() -> Result<()> {
    ...
    // Dual GEMM MLP megakernel
    println!("\nDual GEMM MLP: RMSNorm→GEMM(gate,up)→SiLU×Up");
    run_dual_fused()?;
...
fn run_dual_fused() -> Result<()> {
    ...
    std::fs::write("/tmp/ferrite_dual_fused.ptx", &ptx).ok();
    println!("  [debug] PTX dumped to /tmp/ferrite_dual_fused.ptx");

    let ptx_cstr = CString::new(ptx.as_bytes()).context("PTX null")?;
    let module = unsafe { cuda::module::load_data(ptx_cstr.as_ptr() as *const _)? };
    let func = unsafe {
        cuda::module::get_function(
            module,
            CString::new("fused_rmsnorm_dual_gemm_silu_mul").unwrap(),
        )?
    };
    ...
    let bm: u32 = config.bm;
    let bn: u32 = config.bn;
    let threads: u32 = config.threads();

    // Dual GEMM smem: A(stages) + B0(stages) + B1(stages) + norm_scratch + norm_factors + gamma
    let a_smem = config.smem_a_bytes() * config.num_stages;
    let b_smem = config.smem_b_bytes() * config.num_stages;
    let dual_gemm_smem = a_smem + 2 * b_smem;
    let smem_bytes: c_uint =
        (dual_gemm_smem + 32 + bm * 4 + 4096 * 2) as c_uint;
    ...
    let m: u32 = 256;
    let k: u32 = 4096;
    let n: u32 = 4096; // intermediate size

    println!("  --- batch={}, hidden={}, intermediate={} ---", m, k, n);
    ...
    let d_input = unsafe { cuda::malloc_sync(sa * 2)? };
    let d_wnorm = unsafe { cuda::malloc_sync(sw * 2)? };
    let d_wgate = unsafe { cuda::malloc_sync(sb * 2)? };
    let d_wup = unsafe { cuda::malloc_sync(sb * 2)? };
    let d_output = unsafe { cuda::malloc_sync(sc * 4)? }; // f32 output
    ...
    let h_wnorm: Vec<half::f16> = (0..sw)
        .map(|i| half::f16::from_f32(0.5 + ((i % 100) as f32) * 0.01))
        .collect();
    let h_wgate: Vec<half::f16> = (0..sb)
        .map(|i| half::f16::from_f32(((i % 5) as f32 - 2.0) * 0.1))
        .collect();
    let h_wup: Vec<half::f16> = (0..sb)
        .map(|i| half::f16::from_f32(((i % 11) as f32 - 5.0) * 0.05))
        .collect();
    ...
    unsafe {
        ...
        cuda::memcpy_htod_sync(d_wnorm, &h_wnorm)?;
        cuda::memcpy_htod_sync(d_wgate, &h_wgate)?;
        cuda::memcpy_htod_sync(d_wup, &h_wup)?;
    }
    ...
    let params: &mut [*mut c_void] = &mut [
        ...
        (&d_wnorm) as *const _ as *mut c_void,
        (&d_wgate) as *const _ as *mut c_void,
        (&d_wup) as *const _ as *mut c_void,
        (&d_output) as *const _ as *mut c_void,
        (&n) as *const _ as *mut c_void,
        (&k) as *const _ as *mut c_void,
    ];

    // Use null stream for correctness (blocking sync with memcpy)
    ...
    // CPU reference: output = silu(input_normed @ w_gate) * (input_normed @ w_up)
    let mut max_err: f32 = 0.0;
    for r in [0usize, 1, 31, 63, 64, 127, 128, 255] {
        if r >= m as usize {
            continue;
        ...
        for c in [0usize, 1, 31, 63, 64, 127, 511, 1023, 4095] {
            ...
            // gate = normed @ w_gate[:, c]
            let mut gate_dot = 0.0f32;
            for kk in 0..k as usize {
                gate_dot += normed[kk] * h_wgate[kk * n as usize + c].to_f32();
            }
            // up = normed @ w_up[:, c]
            let mut up_dot = 0.0f32;
            for kk in 0..k as usize {
                up_dot += normed[kk] * h_wup[kk * n as usize + c].to_f32();
            }
            let expected = silu(gate_dot) * up_dot;
            let got = h_output[r * n as usize + c];
            let err = (got - expected).abs();
            if err > max_err {
                max_err = err;
    ...
    if max_err < 3.0 {
        ...
        for r in [0, 1] {
            ...
            for c in [0, 1, 64, 127] {
                ...
                let got = h_output[r * n as usize + c];
                println!(
                    "    C[{r}][{c}]: got={got:.4} expected={expected:.4} err={:.4}",
                    (got - expected).abs()
                );
            }
        }
        bail!("  Max error: {:.4} (expected < 3.0 for fp16 dual GEMM with spills)", max_err);
    }
    ...
    // TFLOPS: dual GEMM = 2 * (2*M*N*K) FLOPs
    let flops = 2.0 * 2.0 * m as f64 * n as f64 * k as f64;
    let tflops = flops / (us * 1e-6) / 1e12;
    println!("  {:.1} us, {:.1} TFLOPS (dual GEMM equiv)", us, tflops);
