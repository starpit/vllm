...
fn emit_kernel_inner(
    ...
    let wy = b
        .build_int_unsigned_div(wid, ci(warps_n as u64), "")
        .unwrap();
    let wx = b
        .build_int_unsigned_rem(wid, ci(warps_n as u64), "")
        .unwrap();
    ...
    let acc_phi_ty = if acc_f16 {
        i32_ty.as_basic_type_enum()
    } else {
        f32_ty.as_basic_type_enum()
    };
    ...
    for ch in 0..(pa / 8) as u64 {
        ...
        let ai = b
            .build_int_add(b.build_int_mul(agr, k_p, "").unwrap(), agc, "")
            .unwrap();
        ...
        if do_cp {
            unsafe {
                emit_cp_async_16(&b, &ctx, &module, sg, ag);
            }
        } else {
            let v = b.build_load(f16_ty.vec_type(8), ag, "").unwrap();
            b.build_store(sg, v).unwrap();
        }
    }
    // Load B
    for ch in 0..(pb / 8) as u64 {
        ...
        let bi = b
            .build_int_add(b.build_int_mul(bgr, n_p, "").unwrap(), bgc, "")
            .unwrap();
        ...
        if do_cp {
            unsafe {
                emit_cp_async_16(&b, &ctx, &module, sg, bg);
            }
        } else {
            let v = b.build_load(f16_ty.vec_type(8), bg, "").unwrap();
            b.build_store(sg, v).unwrap();
        }
    }
    if do_cp {
        unsafe {
            emit_cp_async_commit_wait(&b, &ctx, &module);
        }
    }
    ...
    let mut cur: Vec<inkwell::values::BasicValueEnum> =
        (0..num_acc).map(|i| acc_phis[i].as_basic_value()).collect();
    ...
    for rm in 0..reg_m as u64 {
        ...
        for (ra, ca) in [(0u64, 0u64), (8, 0), (0, 8), (8, 8)] {
            let fr = b
                .build_int_add(b.build_int_add(ro, grp, "").unwrap(), ci(ra), "")
                .unwrap();
            let fc = b.build_int_add(tg2, ci(ca), "").unwrap();
            let lin = b
                .build_int_add(b.build_int_mul(fr, ci(bk as u64), "").unwrap(), fc, "")
                .unwrap();
    ...
    for rn in 0..reg_n as u64 {
        let bcol = b
            .build_int_add(b.build_int_add(wxo, ci(rn * 8), "").unwrap(), grp, "")
            .unwrap();
        let mut bf = Vec::new();
        for fk in [0u64, 8] {
            ...
            let l0 = b
                .build_int_add(b.build_int_mul(k0, ci(bn as u64), "").unwrap(), bcol, "")
                .unwrap();
            let l1 = b
                .build_int_add(b.build_int_mul(k1, ci(bn as u64), "").unwrap(), bcol, "")
                .unwrap();
            ...
            let vec = b
                .build_insert_element(v2f16.get_undef(), v0, ci(0), "")
                .unwrap();
            ...
            if acc_f16 {
                ...
                let [d0, d1, d2, d3] = unsafe {
                    emit_mma_f32(
                        ...
                        &[bf[0], bf[1]],
                        &[
                            cur[ai].into_float_value(),
                            cur[ai + 1].into_float_value(),
                            cur[ai + 2].into_float_value(),
                            cur[ai + 3].into_float_value(),
                        ],
                    )
                };
                cur[ai] = d0.into();
    ...
    let out_elem_ty = if acc_f16 {
        f16_ty.as_basic_type_enum()
    } else {
        f32_ty.as_basic_type_enum()
    };
    for rm in 0..reg_m {
        for rn in 0..reg_n {
            if acc_f16 {
                ...
                for reg_idx in 0..2u32 {
                    let packed = acc_phis[ai + reg_idx as usize]
                        .as_basic_value()
                        .into_int_value();
                    let vec = b
                        .build_bit_cast(packed, v2f16, "")
                        .unwrap()
                        .into_vector_value();
                    for elem in 0..2u32 {
                        ...
                        let mro = if d < 2 {
                            ...
                            b.build_int_add(grp, ci(8), "").unwrap()
                        };
                        let mco = if d % 2 == 0 {
                            tg2
                        } else {
                        ...
                        let cr_ = b
                            .build_int_add(
                                b.build_int_add(br, wyo, "").unwrap(),
                                b.build_int_add(ci(rm as u64 * 16), mro, "").unwrap(),
                                "",
                            )
                            .unwrap();
                        let cc = b
                            .build_int_add(
                                b.build_int_add(bc, wxo, "").unwrap(),
                                b.build_int_add(ci(rn as u64 * 8), mco, "").unwrap(),
                                "",
                            )
                            .unwrap();
                        let ci_ = b
                            .build_int_add(b.build_int_mul(cr_, n_p, "").unwrap(), cc, "")
                            .unwrap();
...
fn bench_one_inner(
    ...
    let n: u32 = 1024;
    let k: u32 = 1024;
    if m % bm != 0 || n % bn != 0 || k % bk != 0 || wm < 16 || wn < 8 {
        return Ok(None);
    }
    if bm % wm != 0 || bn % wn != 0 {
        return Ok(None);
    ...
    let per_a = (bm * bk) / threads;
    let per_b = (bk * bn) / threads;
    if per_a < 8 || per_b < 8 || per_a % 8 != 0 || per_b % 8 != 0 {
        return Ok(None);
    }
    ...
    for _ in 0..3 {
        unsafe {
            cuda::launch_kernel(func, (gx, gy, 1), (threads, 1, 1), smem, stream, params)?;
        }
    }
    unsafe {
    ...
    if acc_f16 {
        unsafe {
            cuda::memcpy_dtoh_sync(&mut hc_f16, dc)?;
        }
        for &r in &check_rows {
            if r >= m as usize {
                continue;
            ...
            for &c in &check_cols {
                ...
                if (hc_f16[r * n as usize + c].to_f32() - e).abs() > 2.0 {
                    ok = false;
                    break;
        ...
        unsafe {
            cuda::memcpy_dtoh_sync(&mut hc_f32, dc)?;
        }
        for &r in &check_rows {
            if r >= m as usize {
                continue;
            ...
            for &c in &check_cols {
                ...
                if (hc_f32[r * n as usize + c] - e).abs() > 1.0 {
                    ok = false;
                    break;
                }
            }
...
pub fn run_sweep(sm: &str) -> Result<()> {
    ...
    println!(
        "\n  ★ Overall best: {} — {:.1} TFLOPS ({:.1}% of cuBLAS)",
        best_name,
        best,
        best / 162.0 * 100.0
    );
