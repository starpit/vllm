...
impl RmsNormAtom {
    ...
    pub fn new(
        ...
        let scratch = RmsNormAtomScratch {
            nf_packed_row0: ptx.regs.alloc_b32(),
    ...
    fn load_norm_factors(&self, ptx: &mut PtxBuilder, rm: u32) {
        ...
        let (nf_packed, nf_packed8) = if rm == 0 {
            (s.nf_packed_row0, s.nf_packed_row8)
        } else {
...
impl TransformAtom for RmsNormAtom {
    fn emit_prologue(&self, ptx: &mut PtxBuilder) {
        ptx.comment("RmsNormAtom prologue: load norm factors into REGISTERS");
        // Load norm factors for rm=0 and rm=1 (REG_M=2)
        self.load_norm_factors(ptx, 0);
        self.load_norm_factors(ptx, 1);
    }
