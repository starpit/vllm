...
/// A graph of `Region`s connected by control edges (barriers /
/// data-dependent dispatch). Represents a whole forward pass (or
/// any self-contained compute graph) at the stencil level.
///
/// The name is intentionally lowering-agnostic: this same graph
/// lowers to *many* kernel launches on the Rust-side dispatch
/// path (one per Region), and could lower to a single persistent
/// `__global__` on a megakernel path. The IR commits to neither.
#[derive(Debug, Clone)]
...
pub fn validate(region: &Region) -> Result<(), String> {
    ...
    // (checked at the RegionGraph level, not here).
