...
template <typename config, typename globals> struct attention_partial {
    ...
    using q_rt = kittens::rt_bf<16, globals::head_dim>; // only 4 rows are used
    using q_st = kittens::st_bf<16, globals::head_dim>; // only 4 rows are used
    ...
    using attn_fl_rt =
        kittens::rt_fl<16, globals::kv_block_size>; // only 4 values are used
    using attn_bf_rt =
        kittens::rt_bf<16, globals::kv_block_size>; // only 4 values are used
    using max_vec_rv =
        col_vec<kittens::rt_fl<16, globals::head_dim>>; // only 4 values are used
    ...
    using o_rt = kittens::rt_fl<16, globals::head_dim>; // only 4 rows are used
...
#pragma unroll
    ...
    // Shape-generic single-warp Q load. Issues `(GQA_RATIO * head_dim *
    // sizeof(bf16)) / (32 lanes * 16 bytes)` cp.async.cg ops per thread.
    //
    // Llama-3.2-1B (GQA_RATIO=4, head_dim=64): 4*64*2 / (32*16) = 1 op/lane,
    //   matches the original MK 1.0 single-call-per-thread fast path.
    // Llama-3.1-8B (GQA_RATIO=4, head_dim=128): 4*128*2 / (32*16) = 2 ops/lane.
    // The unrolled loop generalises without changing the 1B codegen.
    __device__ static inline void
    load_Q_async(q_st &dst, const globals::activations_t &src,
                 const int q_head_start_idx /*0, GQA_RATIO, 2*GQA_RATIO, ...*/) {
        using T = typename q_st::dtype;
        constexpr int elem_per_memcpy =
            sizeof(float4) / sizeof(T);                                      // 8 bf16
        constexpr int bytes_per_lane_op = 16;                                // float4
        constexpr int total_bytes = GQA_RATIO * globals::head_dim * sizeof(T);
        constexpr int total_lane_ops = total_bytes / (32 * bytes_per_lane_op);
        static_assert(total_bytes % (32 * bytes_per_lane_op) == 0,
                      "Q tile size must be a multiple of warp-wide 16B copy.");
        constexpr int memcpy_per_row = globals::head_dim / elem_per_memcpy;
        ...
        uint32_t dst_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(
            &dst.data[(q_head_start_idx % 16) * globals::head_dim]));

        int laneid = kittens::warp::laneid();

#pragma unroll
        for (int op = 0; op < total_lane_ops; ++op) {
            int linear_lane = op * 32 + laneid;
            int row = linear_lane / memcpy_per_row;
            int col = (linear_lane * elem_per_memcpy) % globals::head_dim;
    ...
    struct launcher {
        static __device__ void wait_for_kv(const globals &g, megakernel::state<config> &s,
            ...
            while (*(volatile int *)&g.Bar[{
                       ...
                       globals::num_attention_heads + inst.kv_head_idx}] < 4) {
                ...
                *(volatile int *)&g
                     .Bar[{inst.layer_idx, OPCODE_RMS_QKV_MatVecRopeAppend - 1,
                           globals::num_attention_heads +
                               globals::num_kv_heads + inst.kv_head_idx}] < 4) {
                __nanosleep(config::GMEM_SPIN_LOOP_SLEEP_NANOS);
            }
        ...
        static __device__ void run(const globals &g, megakernel::state<config> &s) {
            ...
            if (kittens::warpid() == 0) {
                ...
                for (int i = 0; i + start_blk_idx < end_blk_idx; ++i) {
                    ...
                    if ((i + start_blk_idx + 1) * globals::kv_block_size >
                        seq_len)
