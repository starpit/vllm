...
// Removed file-scope `using globals = llama_1b_globals;` and the
// dependent file-scope aliases (l_partial_sv / o_sv / o_rv /
// o_final_sv). Pinning them to a concrete `globals` typedef shadows
// the outer template's `Globals` parameter and forces a single shape.
// The aliases now live inside the template, parameterized on `Globals`.

constexpr int Q_HEADS_PER_INSTRUCTION = 4;

template <typename Config, typename Globals> struct attention_reduction {
    static constexpr int MAX_ATTN_PARTIALS = Globals::sm_count;
    static constexpr int ROUNDED_MAX_ATTN_PARTIALS =
        ((MAX_ATTN_PARTIALS + 15) / 16) * 16;
