...
// Removed: `using globals = llama_1b_globals;` — pinning the
// pipeline_specifics signatures to a Llama-1B-specific typedef breaks
// any other shape instantiation. The outer template's `Globals`
// parameter is the source of truth; pipeline_specifics::load_iter etc.
// take `const Globals &g`.
