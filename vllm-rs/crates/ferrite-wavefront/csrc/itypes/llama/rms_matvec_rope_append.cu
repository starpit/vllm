...
// Removed: `using globals = llama_1b_globals;` — see rms_lm_head.cu
// for the rationale. The outer template's `Globals` parameter is the
// source of truth.
...
template <typename Config, typename Globals> struct rms_qkv_rope_append {
    ...
    // Fused QKV row layout: Q rows = num_attention_heads * head_dim,
    // K rows = num_kv_heads * head_dim, V rows = num_kv_heads * head_dim.
    // K starts after Q; V starts after Q + K. Llama-1B (32q×8kv, hd=64):
    // K_BLK_START = 2048 / 16 = 128, V_BLK_START = 2560 / 16 = 160.
    // Llama-3.1-8B (32q×8kv, hd=128): K = 4096/16 = 256, V = 5120/16 = 320.
