...
// Removed: `using config = default_config;` — pinning all
// pipeline_specifics to one Config typedef would force every
// instantiation against default_config; outer template's `Config` is
// the source of truth.
