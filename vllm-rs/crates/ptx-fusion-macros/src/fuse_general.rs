...
fn choose_handoff(proto_a: &KernelProtocol, proto_b: &KernelProtocol) -> HandoffKind {
    let a_elementwise =
    ...
    if a_elementwise && b_elementwise {
        // Both elementwise — register handoff is likely valid.
        // TODO: verify matching thread-to-element mapping by comparing
        // address computation chains (blockIdx.x * blockDim.x + threadIdx.x).
        // For now, use SMEM as the safe default until mapping analysis is wired up.
        HandoffKind::SharedMemory
    } else {
        HandoffKind::SharedMemory
    }
}
...
pub fn fuse_two(
    ...
    let handoff = choose_handoff(&proto_a, &proto_b);

    match handoff {
        HandoffKind::SharedMemory | HandoffKind::Register => fuse_smem(
            &ptx_a_single,
            &ptx_b_single,
            &proto_a,
            &proto_b,
...
mod tests {
    ...
    #[test]
    fn choose_handoff_elementwise_pair() {
        // Simulate two elementwise kernels
        let proto = KernelProtocol {
            name: "test".to_string(),
            registers: vec![],
            smem_regions: vec![],
            total_smem_bytes: 0,
        ...
        // Both elementwise → currently defaults to SMEM (safe)
        assert_eq!(choose_handoff(&proto, &proto), HandoffKind::SharedMemory);
    }

    #[test]
    fn choose_handoff_gemm_elementwise() {
        let gemm_proto = KernelProtocol {
            name: "gemm".to_string(),
            registers: vec![],
            smem_regions: vec![crate::parser::SmemRegion {
                name: "smem".to_string(),
                align: 16,
        ...
        assert_eq!(
            choose_handoff(&gemm_proto, &elem_proto),
            HandoffKind::SharedMemory
        );
    }
}
