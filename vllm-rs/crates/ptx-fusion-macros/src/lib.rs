...
/// Two-phase MLP pipeline: rms_norm+GEMM → barrier → SiLU+mul+GEMM.
///
/// Usage:
/// ```ignore
/// const MLP_PTX: &str = ptx_fusion_macros::mlp_pipeline_fuse!(
///     "kernels/vllm_rms_norm.ptx",
...
/// ```
///
/// Arguments: rms_norm PTX, silu_mul PTX, CUTLASS GEMM PTX, fused name
#[proc_macro]
pub fn mlp_pipeline_fuse(input: TokenStream) -> TokenStream {
    let args: Vec<_> = input.into_iter().collect();
    let mut strings = Vec::new();
    for arg in &args {
        if let proc_macro::TokenTree::Literal(lit) = arg {
    ...
    if strings.len() != 4 {
        return quote! { compile_error!("mlp_pipeline_fuse! expects 4 string args: rms_ptx, silu_ptx, gemm_ptx, name") }.into();
    }

    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let kernel_base = manifest_dir.parent().unwrap().join("ptx-fusion");

    let read_ptx = |path: &str| -> String {
        let full = kernel_base.join(path);
        std::fs::read_to_string(&full)
            .unwrap_or_else(|e| panic!("cannot read {}: {e}", full.display()))
    };
    let rms_ptx = read_ptx(&strings[0]);
    let silu_ptx = read_ptx(&strings[1]);
    let gemm_ptx = read_ptx(&strings[2]);
    let fused_name = &strings[3];

    // Derivations JSON (same GEMM config for both phases)
    let gemm_full = kernel_base.join(&strings[2]);
    let deriv_path = gemm_full.with_extension("derivations.json");
    let deriv_json = std::fs::read_to_string(&deriv_path)
        .unwrap_or_else(|e| panic!("cannot read {}: {e}", deriv_path.display()));

    // Phase 1: rms_norm + GEMM
    let rms_stage = pipeline::PipelineStage::from_ptx("rms_norm", &rms_ptx)
        .unwrap_or_else(|e| panic!("mlp_pipeline: rms_norm analysis failed: {e}"));
    let gemm_stage = pipeline::PipelineStage::from_ptx("gemm", &gemm_ptx)
        .unwrap_or_else(|e| panic!("mlp_pipeline: gemm analysis failed: {e}"));
    let phase1_raw = pipeline_compile::fuse_reduction_into_gemm(&rms_stage, &gemm_stage, "phase1")
        .unwrap_or_else(|e| panic!("mlp_pipeline: phase1 fusion failed: {e}"));
    let (phase1, _) = perimeter::replace_perimeter(&phase1_raw, &deriv_json, "phase1")
        .unwrap_or_else(|e| panic!("mlp_pipeline: phase1 perimeter failed: {e}"));
    let phase1 = dedup_reg_declarations(&phase1);

    // Phase 2: SiLU+mul + GEMM
    let silu_stage = pipeline::PipelineStage::from_ptx("silu_mul", &silu_ptx)
        .unwrap_or_else(|e| panic!("mlp_pipeline: silu_mul analysis failed: {e}"));
    let gemm_stage2 = pipeline::PipelineStage::from_ptx("gemm2", &gemm_ptx)
        .unwrap_or_else(|e| panic!("mlp_pipeline: gemm2 analysis failed: {e}"));
    let phase2_raw =
        pipeline_compile::fuse_pointwise_into_gemm(&silu_stage, &gemm_stage2, "phase2")
            .unwrap_or_else(|e| panic!("mlp_pipeline: phase2 fusion failed: {e}"));
    let (phase2, _) = perimeter::replace_perimeter(&phase2_raw, &deriv_json, "phase2")
        .unwrap_or_else(|e| panic!("mlp_pipeline: phase2 perimeter failed: {e}"));
    let phase2 = dedup_reg_declarations(&phase2);

    // Assemble two-phase kernel
    let assembled = pipeline_compile::assemble_two_phase_kernel(&phase1, &phase2, fused_name)
        .unwrap_or_else(|e| panic!("mlp_pipeline: assembly failed: {e}"));

    quote! { #assembled }.into()
}
