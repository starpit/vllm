...
const CUTLASS_CONFIGS: &[CutlassConfig] = &[
    CutlassConfig {
        ...
        align_b: 8,
        threads: 128,
        smem_bytes: 24576, // (64*32 + 64*32) * 2 * 3 stages
    },
    CutlassConfig {
        ...
        align_b: 8,
        threads: 128,
        smem_bytes: 36864, // (64*32 + 128*32) * 2 * 3 stages
    },
    CutlassConfig {
        ...
        align_b: 8,
        threads: 128,
        smem_bytes: 49152, // (128*32 + 128*32) * 2 * 3 stages
    },
    CutlassConfig {
        ...
        align_b: 8,
        threads: 128,
        smem_bytes: 98304, // (128*64 + 128*64) * 2 * 3 stages
    },
];
...
"#,
    ...
    // Write ferrite.toml manifest (always, from config metadata)
    write_ferrite_toml(&kernel_dir, CUTLASS_CONFIGS);

    if find_cutlass_include().is_none() {
...
fn write_ferrite_toml(kernel_dir: &std::path::Path, configs: &[CutlassConfig]) {
    let toml_path = kernel_dir.join("ferrite.toml");
    let mut toml = String::from("# Auto-generated by build.rs — do not edit\n\n");

    for cfg in configs {
        let (tm, tn, tk) = cfg.tb_shape;
        // Short name: strip prefix and suffix for ergonomic references
        // "cutlass_bf16_64x128x32_sm89" → "gemm_64x128x32"
        let short = format!("gemm_{tm}x{tn}x{tk}");

        toml.push_str(&format!("[kernels.{short}]\n"));
        toml.push_str(&format!("ptx = \"{}.ptx\"\n", cfg.name));
        toml.push_str(&format!(
            "derivations = \"{}.derivations.json\"\n",
            cfg.name
        ));
        toml.push_str(&format!("tile = [{tm}, {tn}, {tk}]\n"));
        toml.push_str(&format!("threads = {}\n", cfg.threads));
        toml.push_str(&format!("smem = {}\n", cfg.smem_bytes));
        toml.push('\n');
    }

    toml.push_str("[intrinsics]\n");
    toml.push_str("rms_norm = true\n");
    toml.push_str("silu = true\n");
    toml.push_str("gelu = true\n");
    toml.push_str("relu = true\n");

    std::fs::write(&toml_path, &toml).expect("write ferrite.toml");
    println!(
        "cargo:warning=wrote ferrite.toml ({} configs)",
        configs.len()
    );
}
