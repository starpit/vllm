...
// SM89 (Ada Lovelace) FP8 CUTLASS GEMM tile configurations.
// Ported from Python vLLM, stripped of PyTorch dependencies.
// Dispatches by M and N dimensions to optimal tile shapes.
#pragma once
...
struct sm89_fp8_config_default {
  ...
  static void dispatch(void* out, const void* a, const void* b,
    ...
    using FallbackGemm =
        typename sm89_fp8_fallback_gemm<InType, OutType, Epilogue>::Cutlass2xGemm;

    uint32_t const np2 = next_pow_2(static_cast<uint32_t>(n));
    ...
    if (np2 <= 4096) {
      ...
      return vllm::fallback_cutlass_gemm_caller<
          ...
          FallbackGemm>(out, a, b, m, n, k, lda, ldb, ldc, stream,
...
// Top-level SM89 FP8 dispatch by M dimension.
template <typename InType, typename OutType,
          template <typename, typename> typename Epilogue,
          typename... EpilogueArgs>
inline void cutlass_gemm_sm89_fp8_dispatch(
  ...
  uint32_t const mp2 =
      std::max(static_cast<uint32_t>(16), next_pow_2(static_cast<uint32_t>(m)));

  if (mp2 <= 16) {
    return sm89_fp8_config_M16::dispatch<InType, OutType, Epilogue>(
        out, a, b, m, n, k, lda, ldb, ldc, stream,
        std::forward<EpilogueArgs>(args)...);
  } else if (mp2 <= 32) {
