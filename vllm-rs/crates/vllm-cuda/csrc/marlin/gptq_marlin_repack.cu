/*
 * GPTQ weight repacking for Marlin tile layout — C entry point for vllm-rs FFI.
 *
 * Repacks GPTQ INT4-in-INT32 packed weights from the standard HuggingFace
 * layout [K/pack_factor, N] into Marlin's tiled format required by the
 * fused GEMM kernel.
 *
 * Adapted from Python vLLM's gptq_marlin_repack.cu with torch deps removed.
 */
...
#define MARLIN_CHECK(cond, ...)                              \
  do {                                                       \
    if (!(cond)) {                                           \
      fprintf(stderr, "GPTQ repack check failed: %s\n", #cond); \
      abort();                                               \
    }                                                        \
  } while (0)
...
/// Repack GPTQ INT4 weights from HF layout to Marlin tiled layout.
///
/// b_q_weight: [size_k/pack_factor, size_n] packed INT4 weights (device ptr, u32)
/// perm:       [size_k] permutation for act_order (device ptr, or nullptr)
...
/// has_perm:   true if perm is non-null (act_order)
void gptq_marlin_repack_4bit(
    const uint32_t* b_q_weight, const uint32_t* perm,
    uint32_t* out, int size_k, int size_n, bool has_perm,
