...
impl PersistentBuffers {
    ...
    pub unsafe fn new(
        ...
        Ok(Self {
            input_ids: driver::mem_alloc(max_batch * 4)?,
            positions: driver::mem_alloc(max_batch * 4)?,
            slot_mapping: driver::mem_alloc(max_batch * 8)?,
            cu_seqlens_q: driver::mem_alloc((max_batch + 1) * 4)?,
            seqused_k: driver::mem_alloc(max_batch * 4)?,
            block_table: driver::mem_alloc(max_batch * MAX_BLOCKS_PER_SEQ * 4)?,
            hidden_a: driver::mem_alloc(hidden_bytes)?,
            hidden_b: driver::mem_alloc(hidden_bytes)?,
            residual: driver::mem_alloc(hidden_bytes)?,
            attn_input: driver::mem_alloc(hidden_bytes)?,
            attn_output: driver::mem_alloc(hidden_bytes)?,
            logits: driver::mem_alloc(logits_bytes)?,
            token_ids: driver::mem_alloc(max_batch * 4)?,
            max_batch,
            hidden_size,
            vocab_size,
            dtype,
    ...
    pub unsafe fn residual_tensor(&self, batch_size: usize) -> GpuTensor {
        GpuTensor::new(self.residual, &[batch_size, self.hidden_size], self.dtype)
    }

    /// Get attention input tensor.
    pub unsafe fn attn_input_tensor(&self, batch_size: usize) -> GpuTensor {
        GpuTensor::new(self.attn_input, &[batch_size, self.hidden_size], self.dtype)
    }
...
impl Drop for PersistentBuffers {
    fn drop(&mut self) {
        unsafe {
            ...
            let _ = driver::mem_free(self.hidden_a);
            let _ = driver::mem_free(self.hidden_b);
            let _ = driver::mem_free(self.residual);
            let _ = driver::mem_free(self.attn_input);
            let _ = driver::mem_free(self.attn_output);
            let _ = driver::mem_free(self.logits);
            let _ = driver::mem_free(self.token_ids);
        }
    }
}
...
impl PiecewiseGraphRunner {
        ...
        F: FnMut(GraphPieceType, &PersistentBuffers, &mut GpuDevice) -> Result<()>,
    {
        ...
        // Free leaked blocks from capture
        device.caching.free_leaked_blocks();
    ...
    pub unsafe fn fill_dummy_decode(
        ...
        driver::memset_d8(self.buffers.input_ids, 0, batch_size * 4, stream)?;
        ...
        driver::memset_d8(self.buffers.residual, 0, hidden_bytes, stream)?;
...
mod tests {
    ...
    fn test_graph_piece_type_ordering() {
        // Test that layer indices are properly ordered
        ...
        // Pre-attention pieces should be ordered by layer index
        assert!(matches!(pre_0, GraphPieceType::LayerPreAttn(0)));
        assert!(matches!(pre_1, GraphPieceType::LayerPreAttn(1)));

        // Post-attention pieces should be ordered by layer index
    ...
    fn test_persistent_buffers_structure() {
        // Test that PersistentBuffers has all required fields
        // This is a compile-time test - if it compiles, the structure is correct
        let _check_fields = |buffers: &PersistentBuffers| {
            let _ = &buffers.input_ids;
            let _ = &buffers.positions;
            let _ = &buffers.hidden_a;
            let _ = &buffers.hidden_b;
            let _ = &buffers.residual;
            let _ = &buffers.attn_input;
            let _ = &buffers.attn_output;
            let _ = &buffers.logits;
            let _ = &buffers.token_ids;
        };
    }

    #[test]
    fn test_piecewise_graph_runner_structure() {
        // Test that PiecewiseGraphRunner has all required fields
    ...
    fn test_graph_piece_structure() {
        // Test that GraphPiece has all required fields
    ...
    fn test_piecewise_replay_output_structure() {
        // Test that PiecewiseReplayOutput has all required fields
    ...
    fn test_graph_piece_type_layer_indices() {
        // Test that layer indices can be extracted correctly
        for i in 0..32 {
    ...
    fn test_graph_piece_type_clone() {
        // Test that GraphPieceType can be cloned
    ...
    fn test_graph_piece_type_debug() {
        // Test that GraphPieceType implements Debug
