    ...
    pub unsafe fn forward(
        ...
        let q_proj_out =
            if let (Some(q_a_proj), Some(q_a_ln)) = (&self.q_a_proj, &self.q_a_layernorm) {
                ...
                let q_a = q_a_proj.forward(hidden_states, &mut device.caching);
                ...
                let q =
                    self.q_b_proj
                        .forward(q_normed.view(), &mut device.caching);
                drop(q_normed);
                q
            } else {
        ...
        let kv_b_out =
            self.kv_b_proj
                .forward(kv_a_normed.view(), &mut device.caching);
        drop(kv_a_normed);
        ...
        let result = self
            .o_proj
            .forward(o_input.view(), &mut device.caching);
        drop(o_input);
