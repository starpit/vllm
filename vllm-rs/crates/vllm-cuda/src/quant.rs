...
/// Apply Marlin scale permutation to a flat scale buffer (CPU, in-place).
///
/// Matches Python `marlin_permute_scales(s, size_k, size_n, group_size)`.
///
...
///
/// Matches Python `awq_to_marlin_zero_points()`:
/// 1. Unpack AWQ packed qzeros
/// 2. Undo AWQ interleaving
/// 3. Apply Marlin permutation
/// 4. Re-interleave for Marlin
/// 5. Pack back to u32
pub fn awq_to_marlin_zero_points(
    ...
    // Step 2: undo AWQ interleaving [0,2,4,6,1,3,5,7]
    ...
    // Apply undo-interleave in chunks of 8
    ...
    // Step 3: apply Marlin scale_perm to zero points
    let perm = scale_perm();
    let chunk_size = 64;
    ...
    // Step 4: re-interleave for Marlin dequantizer [0,2,4,6,1,3,5,7]
    let interleave = [0usize, 2, 4, 6, 1, 3, 5, 7];
    let mut tmp8 = [0u8; 8];
    for chunk_start in (0..total).step_by(8) {
