...
#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn test_cuda_correctness_kimi_k2_tiny() {
    // Kimi K2 tiny synthetic — `DeepseekV3ForCausalLM`, 4 layers BF16.
    // Generated by `scripts/make_tiny_kimi_k2.py`. Must pre-exist at
    // `/tmp/kimi-k2-tiny` on the test host.
    //
    // Exercises K2-specific parametric knobs absent from the existing V3 fleet:
    //   - Flat sigmoid+noaux_tc routing (n_group=1, topk_group=1) — V3 tests
    //     all use grouped routing (n_group=8) or no group selection at all.
    //   - first_k_dense_replace=1 (V3 academic-9B uses 1, V3 tiny uses 1, but
    //     here we exercise it together with the K2 routing combo).
    //   - routed_scaling_factor=2.827 — K2's distinguishing scale value.
    //
    // Random weights → random output; this is a regression test, not a
    // semantics test. Threshold=1: position 0 must match Python vLLM, rest
    // are warnings (same convention as V3 tiny).
    run_correctness_test_with_threshold(TestModels::KIMI_K2_TINY_CUDA, "kimi_k2_tiny", 1).await;
}
