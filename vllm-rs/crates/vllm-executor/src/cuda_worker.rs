...
/// The single ferrite-compiled model variant. All architectures (llama,
/// qwen2, qwen3, gemma2, granite, …) flow through `ferrite_forward::try_load`
/// and share the `FerriteWeights` vtable — there are no per-arch code paths
/// in the executor anymore.
enum CudaModel {
...
impl CudaModel {
    ...
    /// Inject NCCL process group into the ferrite forward context for TP.
    #[cfg(feature = "nccl")]
...
impl Worker for CudaWorker {
    ...
    fn load_model(&mut self) -> ExecutorResult<()> {
        ...
        // 7. Construct model — ferrite-forward owns dispatch.
        let tp_world = self.config.tp_world_size;
        let tp_rank = self.config.tp_rank;
        if self.config.pp_size > 1 {
            return Err(ExecutorError::WorkerInit(
                "Pipeline parallelism is not supported on the ferrite cuda backend".into(),
            ));
        }
        let pp_config: Option<vllm_cuda::PpConfig> = None;
        ...
        let model = ferrite_loaded.ok_or_else(|| {
            ExecutorError::WorkerInit(format!(
                "unsupported architecture for cuda-backend: {arch}. \
                 Only ferrite-forward arches are supported. \
                 Confirm the arch is registered via #[forward] in a ferrite-model-* crate \
                 and FERRITE_MODELS includes the matching base stem."
            ))
        })?;
    ...
    fn determine_available_memory(&mut self) -> ExecutorResult<usize> {
        ...
        // - encoder: no lm_head, profiling path mis-shapes the gather
        let is_encoder = self.model.as_ref().is_some_and(|m| m.is_ferrite_encoder());
        if self.uses_ggml || is_encoder {
            let tag = if self.uses_ggml { "GGML" } else { "encoder" };
    ...
    fn compile_or_warm_up_model(&mut self) -> ExecutorResult<()> {
        ...
        // -----------------------------------------------------------------------
        // 1. Capture Monolithic (Full) Decode CUDA Graphs
        // -----------------------------------------------------------------------
        ...
        let should_capture_monolithic = matches!(
            self.config.cuda_graph_mode,
            CudaGraphMode::Full | CudaGraphMode::FullAndPiecewise | CudaGraphMode::FullDecodeOnly
        );
        ...
        if !should_capture_monolithic {
            ...
            if let Some(ctx) = runner.fp8_graph_ctx() {
                vllm_cuda::attention_helpers::set_fp8_graph_ctx(ctx);
            }
            ...
            vllm_cuda::attention_helpers::clear_fp8_graph_ctx();
