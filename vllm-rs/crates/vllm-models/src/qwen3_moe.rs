...
impl Module for Qwen3MoE {
    fn forward(&self, x: &Tensor) -> candle_core::Result<Tensor> {
        ...
        // For each expert with assigned tokens: gather → batched MLP → weight → scatter.
        // Collect per-token weighted outputs into a Vec, then build the output tensor.
        let device = x.device();
        let dtype = x.dtype();
