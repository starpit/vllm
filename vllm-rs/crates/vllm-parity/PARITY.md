...
| [Model Architectures — Encoder / Embedding](#model-architectures--encoder--embedding) | 🟦🟦🟥🟥🟥🟥🟥 | 2 | 0 | 5 |
...
| ModernBERT | ✅ | ✅ |  |
| RoBERTa | ✅ | ❌ |  |
| ColBERT (BERT backbone) | ✅ | ❌ | Requires BERT encoder arch |
| ColBERT (ModernBERT backbone) | ✅ | ✅ |  |
| ColBERT (Jina RoBERTa backbone) | ✅ | ❌ | Requires Jina RoBERTa encoder arch |
| ColQwen3 (Qwen3-VL backbone) | ✅ | ❌ | Requires Qwen3-VL vision-language arch |
...
| Priority scheduling policy | ✅ | ⚠️ | Waiting queue uses priority heap (correct); preemption victim selection always pops tail (FCFS) instead of picking lowest-priority running request |
...
| ~~Ray executor (distributed)~~ | ✅ | ❌ | 🚫 Won't fix — Ray is a Python-native library with no Rust client; replaced by lightweight Rust launcher + NCCL rendezvous |
| External launcher executor | ✅ | ✅ | --distributed-executor-backend external_launcher; TCP store for NCCL ID exchange + memory allreduce; one CudaWorker per process |
