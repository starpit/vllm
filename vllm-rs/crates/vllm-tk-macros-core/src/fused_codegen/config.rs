...
pub struct FusedPrefillConfig {
    ...
    /// If true, the gate_up fused phase is replaced by TWO sequential CUTLASS
    /// GEMM calls: first `up` writes its output to silu_out (β=0 plain store),
    /// then `gate` runs with the LinearCombinationSiluMul epilogue
    /// (D = silu(α*acc) * source) reading the up output back from silu_out.
    /// This gives cuBLAS-quality compute on the dominant 41%-of-time phase
    /// at the cost of doubling A traffic (no dual_accum reuse).
    pub cutlass_gate_up: bool,
    ...
    /// shape gives only 2-2.5 work units per CTA on these small-N problems
    /// (cuBLAS gets 3-3.5x more efficient by picking smaller tiles for them).
    /// gate_up keeps the kernel-wide shape (out_block stays 64).
    pub phase_opt: bool,
...
impl FusedPrefillConfig {
    ...
    /// 256-row CTA, 8 warps × 32, CUTLASS for ALL FOUR GEMM phases.
    /// QKV / o_proj / down_proj use the standard LinearCombination epilogue;
    /// gate_up is replaced by two CUTLASS calls (up + gate-with-SiluMul).
    pub fn rows256_gemm32_dual_1stage_cutlass4() -> Self {
        Self {
            cutlass_down_proj: true,
            cutlass_qkv_o: true,
            cutlass_gate_up: true,
    ...
    /// 256-row CTA, 8 warps × 32, dual-accum, 1-stage, phase_opt enabled.
    /// Same as the current `_large` polyalgo winner, but with per-phase tile
    /// overrides for QKV/o/down (gemm_warp_m=16, out_block=32 → cta_rows=128).
    /// gate_up keeps the kernel-wide rows256_dual_accum shape.
    /// Targets the per-phase efficiency gap shown by the cuBLAS bench:
    /// QKV (3.5x slower), o_proj (2.8x), down (2.4x) all suffer from
    /// work-distribution starvation at the kernel-wide tile.
    pub fn rows256_gemm32_dual_1stage_phase_opt() -> Self {
        Self {
            phase_opt: true,
            ..Self::rows256_gemm32_dual_1stage()
        }
