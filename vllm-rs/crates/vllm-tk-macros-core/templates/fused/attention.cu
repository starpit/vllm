{# FlashAttention-2 prefill phase.
   For 128-row CTAs, wraps the 16-row pass in an outer attn_pass loop.
   Variables:
     attn_passes: number of 16-row passes per CTA (1 for rows16, 8 for rows128)
     nkh: number of KV heads
     nah: number of attention heads
     stage_sz: 2 * kv_tile_bytes (for double-buffering K+V pages)
#}
    // ════ Attention prefill ════
    const int kv_indptr_start = g.prefill_kv_indptr[{seq_idx}];
{%- if attn_passes > 1 %}
    for (int attn_pass = 0; attn_pass < {{ attn_passes }}; attn_pass++) {
    const int attn_row_offset = attn_pass * PFL_Q_ROWS;
    if (abs_q_row + attn_row_offset > q_start + rel_q_row_last) continue;
    const int attn_abs_q_row = abs_q_row + attn_row_offset;
    const int attn_rel_q_row = rel_q_row + attn_row_offset;
{%- else %}
    const int attn_abs_q_row = abs_q_row;
    const int attn_rel_q_row = rel_q_row;
{%- endif %}
    ...
    pfl_q_st &Q_smem = *reinterpret_cast<pfl_q_st*>(__shm);
    {
        ...
        for (int ri = 0; ri < (PFL_Q_ROWS + rows_per_iter - 1) / rows_per_iter; ri++) {
            ...
            if (row < PFL_Q_ROWS && (attn_abs_q_row + row) <= (q_start + rel_q_row_last)) {
    ...
    warp::sync();
    {
        ...
        for (int row = 0; row < PFL_Q_ROWS; row++) {
            if (attn_abs_q_row + row > q_start + rel_q_row_last) break;
