{# Multi-CTA RMSNorm: distribute row blocks across CTAs.
   Variables: same as rmsnorm.cu
   Uses row_tiles, q_start, q_end from kernel scope.
#}
    // ════ RMSNorm ({{ input_global }} → {{ output_global }}) ════
    for (int rb = bid; rb < row_tiles; rb += num_ctas) {
    const int mcta_abs_q_row = q_start + rb * PFL_Q_ROWS;
    for (int qr = 0; qr < PFL_Q_ROWS && (mcta_abs_q_row + qr) < q_end; qr++) {
    {
    ...
    { sv_bf<globals::hidden_dim> &a = *reinterpret_cast<sv_bf<globals::hidden_dim>*>(act_smem);
       warp::load_async(a, {{ input_global }}, {mcta_abs_q_row + qr, 0}); }
    ...
    if (wid == 0) {
        ...
        warp::store({{ output_global }}, r, {mcta_abs_q_row + qr, 0});
    }
