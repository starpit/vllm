{# Multi-CTA RoPE + KV cache append: distribute row blocks across CTAs.
   Variables: same as rope_kv_append.cu
   Uses row_tiles, q_start, q_end from kernel scope.
#}
    // ════ RoPE + KV cache append (multi-CTA) ════
    ...
    {
    const int token_pos = mcta_abs_q_row + qr;
