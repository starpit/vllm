...
/// Timing test: run the multi-layer kernel 100 times and report average.
/// Compare against KVM (5.8ms) and CUDA graphs (4.6ms) baselines.
#[test]
#[ignore = "needs GPU"]
#[cfg(feature = "cuda")]
fn test_fused_multi_layer_timing() {
    ...
    // Set up buffers (same as full layer test but we don't verify correctness)
    let mut b = TestBuffers::new();
    let input_f32 = bf16_roundtrip(&gen_input(HD));
    let mut input_full = vec![0.0_f32; ACT_ROWS * HD];
    input_full[..HD].copy_from_slice(&input_f32);
    ...
    unsafe {
        ...
        eprintln!();
        eprintln!("╔══════════════════════════════════════════════════╗");
        eprintln!("║  Fused Multi-Layer Kernel Timing ({NL} layers)    ║");
        eprintln!("╠══════════════════════════════════════════════════╣");
