    ...
    let mut dispatch_one = |sg: vllm_tk_macros_core::lowering::assignment::SubgraphId| {
        ...
        match imp_name {
            ...
            name if name.starts_with("cutlass_") => {
                ...
                // Determine (N, K, weight_ptr, act_ptr, out_ptr) from the phase.
...
// ── GPU cost sweep → CSV ──────────────────────────────────────
//
// THIS IS THE ENTRYPOINT FOR ADDING A NEW GPU TO THE SOLVER.
//
// Run this test on the target GPU, redirect stdout to a CSV file,
...
//
// The CSV is read at compile time by the forward! macro's solver.
// Without it, the solver falls back to roofline estimates.
...
fn gpu_cost_sweep() {
    ...
    println!("# To regenerate, see instructions in scheduled_megakernel_test.rs");
