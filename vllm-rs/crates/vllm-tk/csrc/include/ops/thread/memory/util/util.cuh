...
__device__ static inline void arrive(semaphore& sem) {
    ...
    // .release.cta provides release semantics: all prior writes are visible to waiters
    asm volatile (
...
#else
    // sm89: no .release qualifier — add explicit release fence before arrive
...
__device__ static inline void wait(semaphore& sem, int kPhaseBit) {
    ...
    // sm89: mbarrier.test_wait has no acquire semantics — add explicit acquire fence
    // so that all writes made before arrive() are visible to this thread after wait().
    asm volatile("membar.cta;\n" ::: "memory");
#endif
}
