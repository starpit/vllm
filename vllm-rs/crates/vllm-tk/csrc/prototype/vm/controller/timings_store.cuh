...
__device__ void inline store_timings(int *timings, int instruction_index, const globals &g) {
    if constexpr (ducks::gl::all<decltype(g.instructions)>)  {
        ...
        asm volatile(
            ...
            : "memory");
#else
        // sm89: direct warp-stride stores to global memory
        int *dst_ptr = &g.timings[kittens::coord<>{(int)(get_worker_id()), instruction_index, 0}];
        for (int i = ::kittens::laneid(); i < config::TIMING_WIDTH; i += WARP_THREADS)
            dst_ptr[i] = timings[i];
        __threadfence();
#endif
    }
...
#else
    // sm89: store_timings uses warp-stride, needs all lanes
    store_timings<config, globals>(timings, instruction_index, g);
#endif
