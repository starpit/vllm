...
template<typename config> struct page {
    ...
    template<typename T=float> __device__ inline auto &as_st() {
...
#else
        static_assert(std::is_same_v<T, float>, "Only float supported for as_st() on sm89. Please run your own reinterpret_cast!");
#endif
        {
...
#else
            for (int _a = 0; _a < count; _a++) arrive(page_finished[pid][i]);
#endif
        }
    }
