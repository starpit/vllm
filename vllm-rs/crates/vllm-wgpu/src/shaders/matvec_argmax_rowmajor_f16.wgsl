...
//   w_packed[col * (K/2) + k_pair] = pack(w[col, 2*k_pair], w[col, 2*k_pair+1])
...
@group(0) @binding(1) var<storage, read> w: array<u32>;  // packed f16×2
...
fn main(
    ...
    if col < params.N {
        ...
        let w_base = col * k_half;

        let kp_end4 = (k_half / 4u) * 4u;
        for (var kp: u32 = 0u; kp < kp_end4; kp = kp + 4u) {
            let v0 = unpack2x16float(w[w_base + kp]);
            let v1 = unpack2x16float(w[w_base + kp + 1u]);
            let v2 = unpack2x16float(w[w_base + kp + 2u]);
            let v3 = unpack2x16float(w[w_base + kp + 3u]);
            let k0 = kp * 2u;
        ...
        for (var kp: u32 = kp_end4; kp < k_half; kp = kp + 1u) {
            let v = unpack2x16float(w[w_base + kp]);
            let k0 = kp * 2u;
