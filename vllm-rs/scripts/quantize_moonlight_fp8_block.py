...
"""Quantize moonshotai/Moonlight-16B-A3B-Instruct to FP8 block-128x128.

Output format matches what `DeepSeekV2Fp8BlockMoELayer::load` and
`Fp8GemmImpl` expect:
...
Moonlight differs from the academic-9B in that it uses a direct `q_proj`
(q_lora_rank=null), so there is no `q_a_proj`/`q_b_proj` to worry about
with `validate_fp8_block_shape`. All tensors in this model are 128-aligned.

Usage:
    python scripts/quantize_moonlight_fp8_block.py
"""
import json
import os
...
OUT_DIR = Path("/tmp/moonlight-16b-a3b-instruct-fp8-block")
...
# `find_matched_target` can resolve any runtime-fused layer names.
...
print()
print("Upload to HuggingFace (starpit account):")
print(f"  huggingface-cli upload starpit/moonlight-16b-a3b-instruct-fp8-block {OUT_DIR} .")
