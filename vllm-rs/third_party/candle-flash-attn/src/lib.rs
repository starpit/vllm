...
// ---------------------------------------------------------------------------
// Paged KV FlashAttention v2 — reads K/V directly from paged block pools
// ---------------------------------------------------------------------------

struct FlashAttnPagedVarLen {
    pub softmax_scale: f32,
    pub max_seqlen_q: usize,
    pub max_seqlen_k: usize,
    pub seqlens_q: Tensor,
...
impl FlashAttnPagedVarLen {
    ...
    >(
        ...
        let (block_table_storage, block_table_layout) = self.block_table.storage_and_layout();
        let block_table_slice = match &*block_table_storage {
            candle::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?,
            _ => candle::bail!("block_table must be a cuda tensor"),
        };
        let block_table_slice = match block_table_layout.contiguous_offsets() {
            Some((o1, o2)) => block_table_slice.slice(o1..o2),
        ...
        if q_rank != 3 {
            candle::bail!("flash-attn-paged expects q of rank 3 (got {q_rank})")
        }
        if k_rank != 4 || v_rank != 4 {
            candle::bail!("flash-attn-paged expects k/v of rank 4 (k: {k_rank}, v: {v_rank})")
        }
        if q_stride[q_rank - 1] != 1 {
            candle::bail!("the last dim of q must be contiguous {q_stride:?}")
        }
        ...
        let (total_q, num_heads, head_size_og) = q_l.shape().dims3()?;
        let (_num_blocks, _page_size, num_heads_k, _head_size_k) = k_l.shape().dims4()?;
        ...
        let batch_size = nseqlens_q - 1;
        let block_table_batch_stride = if block_table_layout.shape().rank() == 2 {
            let (_bs, max_pages) = block_table_layout.shape().dims2()?;
            max_pages as i64
        } else {
            candle::bail!("block_table must be rank 2 [batch_size, max_pages_per_seq]")
        };
        ...
        unsafe {
            ...
            let (seqlens_q_ptr, _guard) = seqlens_q.device_ptr(&stream);
            let (seqlens_k_ptr, _guard) = seqlens_k.device_ptr(&stream);
            let (block_table_ptr, _guard) = block_table_slice.device_ptr(&stream);
            ...
            ffi::run_mha_paged(
                ...
                /* window_size_right */ window_size_right,
                /* softcap */ 0.0,
                /* block_table_ptr */ block_table_ptr as *const i32,
...
impl candle::CustomOp3 for FlashAttnPagedVarLen {
    fn name(&self) -> &'static str {
        "flash-attn-paged-varlen"
    }
    ...
    fn cpu_fwd(
        ...
        candle::bail!("no cpu support for flash-attn-paged")
    }
    ...
    fn cuda_fwd(
        ...
        match q.dtype() {
            candle::DType::F16 => self.cuda_fwd_t::<f16>(q, q_l, k, k_l, v, v_l, false),
            candle::DType::BF16 => self.cuda_fwd_t::<bf16>(q, q_l, k, k_l, v, v_l, true),
            dt => candle::bail!("flash-attn-paged is only supported for f16/bf16 ({dt:?})"),
        }
    }
}

#[allow(clippy::too_many_arguments)]
/// Flash-attention v2 with paged KV cache (variable-length batching).
///
/// Reads K/V directly from a paged block pool, eliminating the need to gather
/// cached K/V into contiguous tensors before attention.
///
/// # Arguments
///
/// * `q` - Query tensor with shape `(total_q, num_heads_q, head_size)`.
/// * `k_cache` - Paged K cache: `(num_blocks, page_block_size, num_heads_kv, head_size)`.
/// * `v_cache` - Paged V cache: `(num_blocks, page_block_size, num_heads_kv, head_size)`.
/// * `seqlens_q` - Cumulative query lengths, shape `(batch_size + 1,)`.
/// * `seqlens_k` - Cumulative KV lengths, shape `(batch_size + 1,)`.
/// * `block_table` - Block table: `(batch_size, max_pages_per_seq)` i32 on GPU.
/// * `page_block_size` - Tokens per page (e.g., 16).
