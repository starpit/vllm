...
def create_arguments_from_task_schedule(tasks: List[Task], new_tokens: int, num_processors: int = 1, enable_timings: bool = False, q_heads: int = 16, global_work_queue: bool = False):
    ...
    if global_work_queue:
        # Sort all instructions by start time
        tasks.sort(key=lambda t: t.start)
        num_instructions = max(t.uid for t in tasks) + 1
        Instructions = torch.zeros((num_instructions, 32), dtype=torch.int32, device='cuda')-1
        Timings = torch.zeros((num_instructions, 128), dtype=torch.int32, device='cuda') if enable_timings else None
        ...
        completion_flag = torch.zeros((num_instructions, new_tokens), dtype=torch.int32, device='cuda')
        for tid, task in enumerate(tasks):
            if task.task_type == "partial":
                partial_arg = make_partial_arg(task)
                Instructions[tid] = partial_arg
            elif task.task_type == "reduction":
                merge_arg = make_merge_arg(task)
                Instructions[tid] = merge_arg
    else:
        num_instructions = max(t.uid for t in tasks) + 1
        ...
        L_scratch = torch.zeros((num_instructions, new_tokens, q_heads), dtype=torch.float32, device='cuda')
        completion_flag = torch.zeros((num_instructions, new_tokens), dtype=torch.int32, device='cuda')
        Timings = torch.zeros((num_processors, max_num_processor_instructions, 128), dtype=torch.int32, device='cuda') if enable_timings else None
    ...
    return Instructions, O_scratch, L_scratch, completion_flag, Timings
...
def create_evil_arguments_from_task_schedule(tasks: List[Task], new_tokens: int, num_processors: int = 1, enable_timings: bool = False, q_heads: int = 16):
    ...
    for pid in range(num_processors):
        for tid, task in enumerate(partial_processor_tasks[pid]):
            ...
            completion_flag[task.uid,:] = 1
        for tid, task in enumerate(merge_processor_tasks[pid]):
            merge_arg = make_merge_arg(task)
            ReductionInstructions[pid, tid] = merge_arg
    return PartialInstructions, ReductionInstructions, O_scratch, L_scratch, completion_flag, PartialTimings, ReductionTimings
