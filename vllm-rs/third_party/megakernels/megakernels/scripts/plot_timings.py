...
COLOR_MAP = {
    ...
    7: palette[6] if len(palette) > 6 else "#e377c2",
    8: palette[7] if len(palette) > 7 else "#7f7f7f",
    9: palette[8] if len(palette) > 8 else "#bcbd22",
    10: palette[9] if len(palette) > 9 else "#17becf",
}
...
def save_gantt_chart_bokeh(
    ...
    for proc in range(num_processors):
        for instr in range(num_instructions):
            instr_type = Instructions[proc, instr].item()
    ...
    if bar_data["left"]:
        ...
        for p_idx, i_idx in unique_proc_instr_pairs:
            if p_idx < Instructions.shape[0] and i_idx < Instructions.shape[1]:
                present_instr_type_codes.add(Instructions[p_idx, i_idx].item())
...
class ScriptConfig(pydra.Config):
    ...
    insfile: Path | None = None
    timfile: Path | None = None
    out: str | None = None
    depth: int = 2
...
def main(config: ScriptConfig):
    if config.infile is not None:
        ...
        instructions = torch.load(config.insfile, map_location="cpu")
        timings = torch.load(config.timfile, map_location="cpu")
