...
// Standalone: no c10 dependency. Define our own CUDA error check macro.
#include <cstdio>
#include <cstdlib>
#ifndef C10_CUDA_CHECK
#define C10_CUDA_CHECK(err) do { cudaError_t e = (err); if (e != cudaSuccess) { fprintf(stderr, "CUDA error %s:%d: %s\n", __FILE__, __LINE__, cudaGetErrorString(e)); abort(); } } while(0)
#endif
