...
int64_t resolve_thread_kv_page_slice_offset(
    ...
    if (partial_block_size) {
        ...
        // reading off the end of the block_table
        int final_row_offset = *partial_block_size - 1;
        if (final_row_offset < 0) final_row_offset = 0;
        int64_t final_thread_row_offset =
          int64_t(ceil_div(final_row_offset, kGmemRowsPerThread)) * kGmemRowsPerThread;
        if (block_row_offset > final_thread_row_offset)
            block_row_offset = final_thread_row_offset;
    }
